...
| D6 | ~~`GRID_AUTHZ_CACHE_TTL_MS` defaults to `0`, i.e. the FGA check cache is off~~ **Paid on `develop` (2026-09), not by this plan**: the default is now `30000` (`lib/authz/resource-check.ts`, `DEFAULT_AUTHZ_CACHE_TTL_MS`), turned on for the same reason this plan needed it — the websocket-scope route paid up to three WorkOS round-trips per request for a grant that changes a few times a year | Register recall runs `listProjects` — N parallel FGA checks — on every Büro turn, and P3.3 re-authorizes every mount on every upgrade | Nothing left to do here. The recall path and the per-mount re-authorization now hit a 30 s cache by default; the revocation window ADR-0054 already accepts (a grant lives 15 min, a mount is re-checked at the next upgrade) widens by that 30 s, which is inside the window the ADR names. |
...
| **FGA fan-out cost.** `listProjects` is N parallel checks with no bulk list-objects | An org with 40 projects pays 40 checks per Büro turn for recall, plus one per mount per upgrade | Over-fetch then filter (P2.4); the check cache is on by default since `develop` 2026-09 (`GRID_AUTHZ_CACHE_TTL_MS=30000`, D6); mount re-authorization is at most `cap` checks | P3.7 latency run; the recall spec asserts the call count |
