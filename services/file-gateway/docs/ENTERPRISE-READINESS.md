...
| H3 | No production identity: NFSv3 `AUTH_NULL` + `dirpath` is fully spoofable; anyone reaching :2049 can `mount /<anyOrg>/<anyUser>`. | **Open (deploy blocker)** — needs a real resolver (BFF-issued **signed mount token**, or NFSv4+Kerberos / SMB session) **and** transport binding (mTLS/VPN-only network) + drop `AUTH_NULL`. Config already refuses `dirpath` outside dev. |
| H4 | Gateway cache key (`fga:<subj>|<rel>|<obj>`) ≠ BFF invalidation key (`fga:<membership>:project:<proj>:`) → revocation never clears the gateway cache (≤ `SharedCacheTTL` stale-allow even when healthy). ADR claim of "shared consistency" was false. | **Partially fixed** — key now includes org; **Open**: converge the gateway on the BFF's membership/project key schema (or a revocation channel) so BFF invalidation covers both; reconcile ADR-0023 text. Lower `SharedCacheTTL` meanwhile. |
| ING | **Async ingest on drive upload is missing** and the whole ingest path is non-durable: `dispatchIngest` is fire-and-forget (Python down at upload → permanent `failed`, manual re-ingest only); the Python job store is in-memory (restart loses jobs). | **Open — next up.** Design agreed: a `document_ingest_queue` + DB-claimed worker (clone `deletion_queue`/purger, ADR-0021), a single `commitDocument` seam both web + drive feed, and a gateway commit-detector (ignore `~$`/`.tmp`/`.ac$`/`.bak`; fire on final rename/close + quiescence debounce). See "Async ingest plan" below. |
