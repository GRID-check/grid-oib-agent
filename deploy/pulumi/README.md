...
| `seaweedfsVolumeMaxCount` | `64` | Writable volume slots a volume server will hold open (`-max`). Applies to **both** topologies — `single` hardcoded 8 until staging ran out of them. A bucket is a SeaweedFS COLLECTION and each one needs a writable volume of its own, so with `seaweedfsPerOrgBuckets` on, this number is the ceiling on how many organizations can ever be onboarded. Hitting it looks nothing like a capacity problem: `/dir/status` reports `Max:8 Free:0`, the new org's collection shows `writables: null`, and its first write fails with a bare S3 `InternalError` while the disk sits all but empty. Slots are a ceiling, not a reservation — SeaweedFS grows volumes lazily, so raising this costs nothing until the volumes are used |
...
| `registerReconcileEnabled` | `true` | Creates the `register-reconcile` CronJob, which calls `POST /api/internal/workspace/register/reconcile`: rebuild the Steckbrief of every project whose register row is missing or stale, oldest first, 50 per tick. Default-**on**, and not housekeeping — this is how the Büro learns a project exists (ADR-0054). Write-through covers a project someone edits and nothing else does, so with this off every organization that predates the register stays invisible to the office and the Büro answers "no matching projects" about a full portfolio |
| `registerReconcileSchedule` | `30 * * * *` | 5-field cron. Hourly, on the half hour: the batch is 50, so the ship-day backfill of a large deployment wants ticks in hours rather than days, and an hour is also the longest a missed write-through should leave a Steckbrief describing a project as it used to be. A tick with nothing to do is one indexed query |
| **Memory** | | |
