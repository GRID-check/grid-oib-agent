...
class TestNormTrustChainStamping:
    """add() stamps NormReference trust-chain fields from the norm registry."""
        ...
        return nr.NormRegistry(
            entries=[
                nr.NormEntry(
                    ...
                    aliases=["OIB-Richtlinie 2"],
                    editions=[
                        nr.Edition(
                            id="2023-05",
                            label="Ausgabe Mai 2023",
                            status="current",
                            source=nr.EditionSource(kind="corpus", file="rl2.pdf"),
                        )
                    ],
                ),
                nr.NormEntry(
                    ...
                    relations=[
                        nr.Relation(type="declares_binding", target="oib-rl-2", edition="2023-05", status="verified")
                    ],
                ),
            ]
        )
    ...
    def _patch_registry(self, registry, monkeypatch):
        monkeypatch.setattr("aiq_agent.cards.registry._load_norm_registry_safe", lambda: registry)
        monkeypatch.setattr("aiq_agent.cards.registry._resolve_bundesland_safe", lambda: "wien")

    def test_stamps_reference_fields(self):
        reg = CardRegistry()
        reg.add({"type": "x", "reference": {"document": "OIB-Richtlinie 2", "edition": "Ausgabe Mai 2023"}})
        reference = reg.snapshot()[0]["reference"]
        assert reference["norm_id"] == "oib-rl-2"
        assert reference["rank"] == "oib_richtlinie"
        assert reference["binding"] == "normativ"
        assert reference["via"] == "WBTV 2023"

    def test_llm_authored_fields_are_overwritten(self):
        reg = CardRegistry()
        reg.add(
            {
                "type": "x",
    ...
    def test_unknown_document_stays_free_text(self):
        reg = CardRegistry()
        reg.add({"type": "x", "reference": {"document": "ÖNORM B 9999", "edition": "2020"}})
        reference = reg.snapshot()[0]["reference"]
        assert "norm_id" not in reference
        assert reference["document"] == "ÖNORM B 9999"

    def test_nested_references_are_stamped(self):
        reg = CardRegistry()
        reg.add(
            {
                "type": "x",
                "items": [
                    {"label": "a", "reference": {"document": "OIB-Richtlinie 2", "edition": "Ausgabe Mai 2023"}},
                    {"label": "b"},
                ],
            }
        )
        items = reg.snapshot()[0]["items"]
        assert items[0]["reference"]["norm_id"] == "oib-rl-2"
        assert "reference" not in items[1]

    def test_registry_load_failure_is_fail_open(self, monkeypatch):
        monkeypatch.setattr("aiq_agent.cards.registry._load_norm_registry_safe", lambda: None)
        reg = CardRegistry()
        reg.add({"type": "x", "reference": {"document": "OIB-Richtlinie 2", "edition": "Ausgabe Mai 2023"}})
        assert "norm_id" not in reg.snapshot()[0]["reference"]
