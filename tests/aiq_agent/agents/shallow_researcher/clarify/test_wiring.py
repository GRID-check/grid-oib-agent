...
from aiq_agent.agents.shallow_researcher.clarify import Clarifier
from aiq_agent.agents.shallow_researcher.clarify import ClarifierSettings
from aiq_agent.agents.shallow_researcher.clarify import ask_through_nat
from aiq_agent.agents.shallow_researcher.clarify import resolve_tools
...
class TestResolveTools:
    ...
    async def test_an_empty_tool_list_inherits_the_registry(self):
        ...
        with patch("aiq_agent.agents.shallow_researcher.clarify.get_all_tool_refs", return_value=["registry_tool"]):
...
class TestAskThroughNat:
    ...
    async def test_a_question_without_options_is_a_text_prompt(self):
        ...
        with patch("aiq_agent.agents.shallow_researcher.clarify.Context.get", return_value=context):
...
class TestDepsForRequest:
    ...
    def test_narrowed_data_sources_produce_their_own_deps(self):
        ...
        with patch("aiq_agent.agents.shallow_researcher.clarify.filter_tools_by_sources", return_value=[alpha_tool]):
    ...
    def test_the_planner_is_carried_through_the_override(self):
        ...
        with (
            patch("aiq_agent.agents.shallow_researcher.clarify.apply_model_override", return_value=planner) as override,
            patch("aiq_agent.agents.shallow_researcher.clarify.apply_org_credential", return_value=planner),
        ):
