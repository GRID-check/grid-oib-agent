...
fresh ``ShallowResearcherAgent``: the 43 KB prompt re-read from disk inside
...
from aiq_agent.agents.shallow_researcher.agent import TurnConfig
from aiq_agent.agents.shallow_researcher.agent import _recursion_limit
...
def counters(monkeypatch):
    ...
    real_build = ShallowResearcherAgent._build_graph
    real_init = ShallowResearcherAgent.__init__
    ...
    monkeypatch.setattr(ShallowResearcherAgent, "_build_graph", counting_build)
    monkeypatch.setattr(ShallowResearcherAgent, "__init__", counting_init)
    yield counts
    prompt_module.system_prompt_template.cache_clear()
...
async def test_two_skills_turns_build_one_agent_read_the_prompt_once_and_compile_once(counters):
    ...
    with (
        ...
        gen = shallow_research_agent.__wrapped__(config, _FakeBuilder(llm))
        run_fn = (await gen.__anext__()).single_fn
        try:
            first = await run_fn(ShallowResearchAgentState(messages=[HumanMessage(content="Wie tief?")]))
            second = await run_fn(ShallowResearchAgentState(messages=[HumanMessage(content="Und wie hoch?")]))
        finally:
            await gen.aclose()
...
async def test_a_turn_that_varies_nothing_reuses_the_boot_binding():
    ...
    agent = ShallowResearcherAgent(llm_provider=provider, tools=[web_search_tool])
    llm.bind_tools.reset_mock()

    await agent.run(ShallowResearchAgentState(messages=[HumanMessage(content="Q")]))
    await agent.run(
        ShallowResearchAgentState(messages=[HumanMessage(content="Q")]),
...
async def test_the_turns_reserve_reaches_the_ceiling_through_the_graph_config():
    ...
    spent = ShallowResearchAgentState(messages=[HumanMessage(content="Wie tief?")], tool_iterations=3)
