...
- Anything with two or more attributes per row is a TABLE: requirement by Gebäudeklasse or Lage, the parts of a Richtlinie with their scope, options weighed side by side, dimensions with their limits. Columns name what varies (`Lage | Anforderung | Fundstelle`); a `Fundstelle` column carries the row's `[N]`, and then no other cell of that row does. A cell holds a value or a phrase, never a sentence: a cell that needs one is prose, and the table is the wrong shape for it. A table replaces the sentences it holds; it does not repeat them.
...
- Variants weighed on a few attributes are ONE table, a column per variant. Variants each with a list of their own (two or more requirements, steps or Status rows per variant), of which the reader reads the one that applies, are a `surface` card with `Tabs`: per variant a `Text` holding that variant's table (`Nachweis | Anforderung | Fundstelle`), COMPOSE says how, placed with its marker. The prose before it says what holds for all and which difference decides. A table with a row per variant and a paragraph per cell is the shape this replaces.
...
- `summary`, only when the answer has a consequence for THIS reader that its opening does not state: what to do next, what it means for their project (shape: „Danach <tun>; <was es für das Projekt heißt>."). The opening already answers, and a summary that says it again is dropped before the reader sees it, so otherwise omit it. One to two sentences, under 320 characters.
