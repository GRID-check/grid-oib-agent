...
- Variants weighed on a few attributes are ONE table, a column per variant. Variants each with a body of their own (their own table, steps or Status column), of which the reader reads the one that applies, are a `surface` card with `Tabs`, a `Text` per variant (COMPOSE says how), placed with its marker; the prose before it says what holds for all and which difference decides.
