...
def _verify(content: str, registry: SourceRegistry) -> _Verified:
    ...
    logger.debug(
        "Shallow researcher: citation verification complete — %d valid, %d removed",
...
def _adopt_if_better(verified: _Verified, repaired: Repair, registry: SourceRegistry) -> _Verified | None:
    ...
    if not (after < verified.failure_count and candidate.valid_citations):
        logger.info("Shallow researcher: repair pass discarded (%d -> %d failures)", verified.failure_count, after)
        return None
    logger.info("Shallow researcher: repair pass adopted (%d -> %d failures)", verified.failure_count, after)
...
def _ground(verified: _Verified, registry: SourceRegistry, *, lookup_attempted: bool) -> _Grounding:
    ...
    if verified.unverified_quotes:
        ...
        logger.info(
            "Shallow researcher: %d quoted span(s) not verbatim in any retrieved passage; annotated inline",
...
def _require_retrieval(lookup_attempted: bool, tools: Sequence[BaseTool]) -> None:
    ...
    if not lookup_attempted:
        logger.debug("Shallow researcher: answered without querying any data-source tool; no verification")
        return
    _, available_count, unavailable = validate_tool_availability(
        list(tools), research_type="shallow research", enable_logging=False
    )
    ...
    raise EmptySourceRegistryError("shallow research", unavailable_tools=unavailable, available_count=available_count)
...
def _gated_meta(extracted: _Extracted, content: str) -> dict[str, Any] | None:
    ...
    Skipped on an escalating turn: the shallow answer is about to be
