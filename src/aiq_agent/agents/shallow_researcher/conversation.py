...
So this module adds exactly two things to :class:`ShallowResearcherAgent`: the
...
def _escalation_update(message: BaseMessage, result: ShallowResearchAgentState) -> dict[str, Any]:
...
def _finalize_answer(message: BaseMessage, result: ShallowResearchAgentState) -> dict[str, Any]:
...
class ConversationGraph:
    ...
    async def _run_shallow(
        ...
        try:
            return await self.shallow_research_fn(shallow_state)
    ...
    async def _shallow_research_node(self, state: ConversationState) -> dict[str, Any]:
