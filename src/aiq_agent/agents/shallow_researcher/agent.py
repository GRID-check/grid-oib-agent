"""Shallow research agent: the bounded tool-calling turn loop.
...
_TURN_BINDING_KEY = "shallow_researcher_turn"
...
def _charge_tool_calls(response: Any, state: ShallowResearchAgentState, ceiling: int) -> tuple[int, int]:
...
def _capture_sources(
    ...
    configured data source: the shallow tool list also carries interaction
...
class ShallowResearcherAgent:
    ...
    Example:
        ...
        >>> agent = ShallowResearcherAgent(llm_provider=provider, tools=[web_search_tool], max_tool_iterations=5)
    ...
    async def _agent_node(self, state: ShallowResearchAgentState, config: RunnableConfig) -> dict[str, Any]:
    ...
    async def _tools_node(self, state: ShallowResearchAgentState, config: RunnableConfig) -> dict[str, Any]:
        ...
        registry = get_session_registry()
        if registry is None:
            raise RuntimeError("ShallowResearcherAgent graph invoked outside run(): no source registry is bound")
