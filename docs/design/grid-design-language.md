...
- **State changes inside an arrived turn are transitions, not entrances**: `AgentPrompt` dims to `opacity-75` via `transition-opacity duration-300` once answered; the answer's meta row fades in on a short `[animation-delay:120ms]` (with `[animation-fill-mode:backwards]`, so nothing flashes before its delay) and the source chips cascade on a 40ms per-chip stagger, capped.
...
- **Every animation carries `motion-reduce:animate-none`** (or is a `transition-*` the browser already reduces). `prefers-reduced-motion` is not a nice-to-have here: motion is decoration in this product, so it must be droppable with no loss of information.
