...
| `AIQ_RERANKER_API_KEY` | No | `OPENROUTER_API_KEY`, then provider inference from the base URL | Explicit reranker key override. Resolved through the shared resolver like the embeddings key; when nothing resolves the cross-encoder is not built and the judge reranks. Org BYOK is **not** wired for reranking (the reranker is built once at startup with no organization in scope) — the platform key is used. |
| `AIQ_RERANKER_TIMEOUT_SECONDS` | No | `3` | Per-request bound on the cross-encoder call. A cross-encoder answers in ~100ms; 3s bounds a hung provider before the judge fallback, so worst case a turn pays 3s + the judge's own 30s bound. Five consecutive failures disable the cross-encoder for 5 min (breaker); per-search failure warnings are throttled to one per 5 min. Rerank spend is currently **untracked** (no ledger row — the call bypasses the LangChain callback path cost tracking hooks); TODO(cost-tracking): emit a usage event per call. |
