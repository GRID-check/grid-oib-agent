...
| `post-answer-follow-ups` *(WorkOS feature flag, not env)* | No | off | Runtime on/off switch for the post-answer follow-up-questions stage **when `GRID_ENFORCE_FEATURE_FLAGS=true`**: evaluated per organization **per turn**, served to the backend by `GET /api/internal/stages`; fail-closed. Without enforcement this flag is ignored and `GRID_STAGE_FOLLOW_UPS_ENABLED` decides instead. The stage runs `silent` — it delivers nothing to the reader, so switching it on buys the measurement (skip rate, empty rate, tokens, wall time), not chips. See `docs/architecture/post-answer-stages.md` §10.
| `GRID_STAGE_FOLLOW_UPS_ENABLED` | No | `false` | Runtime on/off switch for the follow-up-questions stage **when `GRID_ENFORCE_FEATURE_FLAGS` is off** (the default). New stages default OFF: this one costs an LLM call per substantive answer and currently returns nothing to the reader. Frontend service. The backend additionally no-ops when the workflow config sets no `follow_ups_llm` (the capability bit).
