...
| `AIQ_CHECKPOINT_DB` | No (SQLite fallback) | `postgresql://aiq:aiq_dev@postgres:5432/aiq_checkpoints` | `./checkpoints.db` | LangGraph conversation checkpoint database, and the LangGraph store behind the chat working directory (the per-conversation drafts `write_file`/`edit_file` produce). PostgreSQL for Docker, SQLite for local dev — a non-Postgres value keeps the checkpointer on SQLite and puts the working directory in memory, so drafts do not survive a restart of the agent process. |
...
| `AIQ_EMBED_TIMEOUT_SECONDS` | No | `60` | Per-request timeout on the INGESTION embeddings client, with two retries; queries use `AIQ_QUERY_EMBED_TIMEOUT_SECONDS`. The client's own defaults were 120s and five retries, so a hung embeddings endpoint could hold a chat turn for ten minutes; now the retrieval fails open after at most three bounded attempts like every other retrieval step. |
| `AIQ_QUERY_EMBED_TIMEOUT_SECONDS` | No | `3` | Per-request timeout on the query embeddings a chat turn waits on, with two retries. A query embeds one short text (0.66 s median, 8.5 s p99 measured); bounding it tightly and retrying turns an 11 s tail request into about 4 s. |
...
| `AIQ_VLM_MODEL` | No | `openai/gpt-6-luna` | VLM model for vision-language tasks (image captioning during ingestion). |
| `AIQ_VLM_BASE_URL` | No | `https://openrouter.ai/api/v1` | Base URL for the VLM API. Drives provider inference for the VLM key (the default resolves `OPENROUTER_API_KEY`). |
| `AIQ_VLM_API_KEY` | No | the provider key inferred from `AIQ_VLM_BASE_URL` | Explicit VLM key override. Resolution chain: org **BYOK** (for per-project/Archiv uploads — `/v1/ingest` forwards `x-grid-organization-id`) → `AIQ_VLM_API_KEY` → provider inference from the base URL. This one resolver (`resolve_vlm_credential`) is the single source of truth both ingestion and the `vlm_available` capability bit consult; an org's `ingest_vlm` runtime model override is applied on top (see `docs/architecture/org-model-configuration.md`). |
| `AIQ_EMBED_API_KEY` | No | the provider key inferred from `AIQ_EMBED_BASE_URL` | Explicit embeddings key override (read by the adapter). Chain: `AIQ_EMBED_API_KEY` → provider inference. Inference only selects the key for the configured base URL; it never changes the base URL (embeddings need an embeddings-capable endpoint). |
| `AIQ_EMBED_MODEL` | No | `openai/text-embedding-3-large` | Embedding model id. Keep it stable across redeploys: stored vectors are only comparable to query vectors from the same model. |
...
| `GRID_CONV_STREAM_MAXLEN` | No | `2000` | Max frames kept per conversation in the replay stream (`conv:<id>:stream`), bounding Dragonfly memory (approximate trim). A client whose socket dropped mid-answer reads back what it missed from here (`GET /api/conversations/:id/frames`); a streamed turn is a few hundred frames, so this holds a few turns. Too low and a long answer ages out before a phone comes back, which ends as the „Antwort ging verloren" banner. Best-effort — the persisted answer stays source of truth. |
| `GRID_CONV_STREAM_TTL_SECONDS` | No | `3600` | Expiry of a conversation's replay stream, renewed by every frame written to it. Bounds the memory of conversations nobody comes back to. A client away longer than this cannot resume the turn and falls back to fetching the finished answer. |
...
| `AIQ_RERANKER_TIMEOUT_SECONDS` | No | `3` | Per-request bound on the cross-encoder call. A cross-encoder answers in ~100ms; 3s bounds a hung provider before the judge fallback, so worst case a turn pays 3s + the judge's own 30s bound. Five consecutive failures disable the cross-encoder for 5 min (breaker); per-search failure warnings are throttled to one per 5 min. Each successful rerank now leaves a usage event on the turn's ledger with the reranker model and its token count; the endpoint prices in search units and reports no cost, so the row's `cost_source` is `estimate` and its USD stays 0 rather than being invented. |
| `AIQ_RERANKER_MAX_DOC_CHARS` | No | `4000` | Per-passage character budget sent to the reranker. Bounds a pathological chunk; a normal 1024-token chunk fits whole. |
| `CONSISTENCY_LLM_MODEL` | No | `LLM_MODEL`, then `openai/gpt-6-luna` (if `OPENROUTER_API_KEY` set) / `gpt-4o-mini` | Model for the end-of-wizard free-text intake consistency-check endpoint (`POST /v1/consistency-check`). Falls back to the generic `LLM_MODEL`, then the OpenRouter/OpenAI default. |
...
| `SUMMARY_LLM_MODEL` | No | `LLM_MODEL`, then `openai/gpt-6-luna` (if `OPENROUTER_API_KEY` set) / `gpt-4o-mini` | Model for the AI project-summary endpoint (`POST /v1/generate-summary`). Falls back to the generic `LLM_MODEL`, then the OpenRouter/OpenAI default. BYOK never changes the model. |
...
The one-off tag-backfill script runs **outside** the NAT runtime, so it builds an OpenAI-compatible client directly from these env vars, which must match the `summary_llm` block in `configs/config_oib_openrouter.yml`.

| Variable | Required | Default | Description |
|----------|----------|---------|-------------|
| `BACKFILL_SUMMARY_API_KEY` | Yes* | provider inference (`OPENROUTER_API_KEY` for the default base URL) | API key for the tagging LLM, resolved through the shared resolver: `BACKFILL_SUMMARY_API_KEY` → the provider key inferred from `BACKFILL_SUMMARY_BASE_URL`. If none resolves the script exits with code `2` (LLM could not be constructed). The script runs outside NAT, so BYOK does not apply. |
| `BACKFILL_SUMMARY_BASE_URL` | No | `https://openrouter.ai/api/v1` | Base URL for the tagging LLM. Drives provider inference for the tagging key. |
...
| `memory-reflection` *(WorkOS feature flag, not env)* | No | off | Runtime on/off switch for the reflection stage **when `GRID_ENFORCE_FEATURE_FLAGS=true`**: evaluated per organization **per turn**, served to the backend by `GET /api/internal/stages` (`isOrgFeatureEnabled` behind a 30s cache); fail-closed, including org-less/anonymous sessions. It used to be evaluated once at the WebSocket upgrade, which meant switching it off did not reach an already-open tab. Without enforcement this flag is ignored and `GRID_MEMORY_REFLECTION_ENABLED` decides instead. See `docs/architecture/project-memory-design.md` §3.5. |
...
| `post-answer-follow-ups` *(WorkOS feature flag, not env)* | No | **on** | Runtime on/off switch for the post-answer follow-up-questions stage **when `GRID_ENFORCE_FEATURE_FLAGS=true`**: evaluated per organization **per turn**, served to the backend by `GET /api/internal/stages`; fail-closed. Without enforcement this flag is ignored and `GRID_STAGE_FOLLOW_UPS_ENABLED` decides instead. `flagEnabled: true`, `accessType: ALL`, `defaultEnabled: true` in BOTH the Staging and Production WorkOS environments, so a newly created tenant inherits it. Switching it off for an org stops the frames on the next turn, with no deploy and no effect on the answer either way — but it no longer falls back to the in-answer `follow_ups` CARD, which slice 4 retired, so that org has no follow-up questions at all. See `docs/architecture/post-answer-stages.md` §10 and §7.10.
...
| `GRID_IFC_PREVIEW_FIRST` | No | `true` | Whether a click on an `.ifc` opens the FILE PREVIEW first, or goes straight to the full-screen model stage. Read while `GRID_ENFORCE_FEATURE_FLAGS` is off; with enforcement on, the per-org `ifc-preview-first` WorkOS flag decides instead. **On by default** — the preview shows the building and carries an "Im Modellbereich öffnen" button, so the analytical surface is a choice rather than a landing. Set it to `false` to restore the direct jump. Distinct from `GRID_IFC_MODELS_ENABLED`, which decides whether the model surfaces exist at all; this one only decides what a click does when they do. It is read by BOTH file surfaces — a project's Dateien and the org-wide Archiv — from the same helper, so it can only ever move them together: an `.ifc` behaving one way in a project and another in the Archiv is the defect it exists to prevent. Frontend service. |
| `GRID_AGENT_AUTHORED_DOCUMENTS_ENABLED` | No | `true` | Whether Piloti may file the documents it writes — a finished Recherchebericht, a drawn diagram — into a project (`lib/documents/generated.ts`), while `GRID_ENFORCE_FEATURE_FLAGS` is off. **On by default.** Set it to `false` to stop filing for every organization at once, without a redeploy of the previous image and without touching a role. It is deliberately NOT the same lever as the `project:documents:generate` permission: that one is the TENANT's (per project, per role, set by the organization's own admin), and withdrawing it fleet-wide would mean editing the built-in `project-editor`/`project-admin` roles in WorkOS, which then fails `provision:authz --check` in CI. With enforcement on, the per-org `agent-authored-documents` WorkOS flag decides instead. Frontend service. On Kubernetes it is set from the Pulumi stack key `grid-oib:agentAuthoredDocumentsEnabled`. |
...
| `GRID_SKILLS_ENABLED` | No | `false` | Dark-launch gate while `GRID_ENFORCE_FEATURE_FLAGS` is off: `true` shows the Automation section (the Aufgaben and Skills tabs) and enables every `/api/skills/*` and `/api/projects/*/jobs/*` route (`/jobs` keeps its name as the compatibility endpoint over the same definitions), and it is the **start gate for the skill-scheduler container** — when neither this nor flag enforcement is on, the scheduler logs why and exits 0 rather than polling for work nobody can create. Read case-insensitively by both, so the UI and the worker can never disagree. With enforcement on, the per-org `skills` WorkOS flag controls the UI/API instead. Frontend + skill-scheduler services. On Kubernetes it is set from the Pulumi stack key `grid-oib:skillsEnabled` (see `deploy/pulumi/README.md`), not by hand — and that key is also what decides whether the skill-scheduler Deployment is created at all. |
| `GRID_SKILL_SCHEDULER_POLL_MS` | No | `30000` | Scheduler tick interval. Each tick claims due `task_definitions` rows (`FOR UPDATE SKIP LOCKED`), advances `next_run_at`, then fires them through the BFF internal endpoint. skill-scheduler service. |
| `GRID_SKILL_SCHEDULER_BATCH` | No | `20` | Max due definitions claimed per tick. skill-scheduler service. |
| `GRID_SKILL_MIN_INTERVAL_MINUTES` | No | `15` | Minimum cadence a definition's cron expression may have; enforced at save time in the BFF. Frontend service. |
...
| `GRID_ENFORCE_FEATURE_FLAGS` | No | `false` | `true` enforces WorkOS feature flags (`runtime-model-config`, `deep-research` — registry: `lib/authz/feature-flags.ts`). Turn on only after the flags exist in WorkOS and orgs are targeted; sessions without the JWT `feature_flags` claim then fail closed until re-login. **Production runs `true` since 2026-09-18** (`grid-oib:enforceFeatureFlags`), so in that environment every `GRID_*` gate in this table that names a WorkOS half is inert and the flag decides — including the default-ON ones, which stop defaulting to ON. Staging is still `false`. Frontend + skill-scheduler services. It reached only the **scheduler** in compose until 2026-09-10, so `enforcementOn()` in the BFF read an unset variable however the operator had configured it, and the two containers could disagree about whether flags decided anything; both compose files now pass it to the frontend as well. Its effective value is on the `[boot]` line, so a deployment no longer has to be trusted about it. |
