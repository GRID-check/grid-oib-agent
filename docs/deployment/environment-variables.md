...
| `RIS_CATALOG_PATH` | No | `configs/ris_catalog.yml` | Path of the curated RIS index — verified pointers to the building-relevant Austrian norms (nine state building codes, Wiener Garagengesetz, adjacent federal acts). Consumed by the `ris_search` catalog short-circuit, the `ris_catalog_lookup` tool, and the researcher prompt block. Missing/invalid file disables the feature with a warning; live RIS search is unaffected (fail-open). Regenerate via `scripts/build_ris_catalog.py` (verifies every entry against the live OGD-RIS API). |
