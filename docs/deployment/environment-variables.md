...
| `AIQ_VLM_TIMEOUT_SECONDS` | No | `180` | Per-request timeout on the VLM OpenAI client (both image and drawing call sites), with a single retry. Previously SDK defaults (≈600s × 2 retries) let one hung provider park an ingest worker for ~20 minutes. |
