...
| `AIQ_VLM_MODEL` | No | `openai/gpt-5.6-luna` | VLM model for vision-language tasks (image captioning during ingestion). |
...
| `BACKFILL_SUMMARY_MODEL` | No | `GRID_DEFAULT_MODEL`, then `openai/gpt-5.6-luna` | Tagging model id. Match the config's `summary_llm` model. |
