...
| `AIQ_RERANKER_PROVIDER` | No | `none` | Cross-encoder reranking provider for `knowledge_search`: `none` \| `openrouter` \| `cohere` \| `voyage` \| `jina` \| `nvidia`. The env value is the default the config's `reranker_provider` field falls back to; the reference config sets `openrouter`. Any provider makes the cross-encoder primary and the `rerank_llm` judge the fallback; `none` hands reranking back to the judge. |
| `AIQ_RERANKER_MODEL` | No | provider-specific (`cohere/rerank-v3.5` on OpenRouter) | Reranking model id. The corpus is German, so a multilingual reranker is not optional. |
| `AIQ_RERANKER_BASE_URL` | No | provider-specific | Override the provider's host (a self-hosted NIM serves `/v1/ranking`, a gateway, a test double). |
| `AIQ_RERANKER_API_KEY` | No | the provider's conventional key (`OPENROUTER_API_KEY` for `openrouter`), then provider inference from the base URL | Explicit reranker key override. Resolved through the shared resolver like the embeddings key; when nothing resolves the cross-encoder is not built and the judge reranks. |
