...
| `GRID_ATTACHMENT_INDEX_WAIT_SECONDS` | No | `8` | Seconds a chat turn may wait for a file the user just attached to appear in `available_documents` (ADR-0048). Only a wire-declared `indexing` attachment the inventory does not list is waited for, re-read every 500 ms. `0` disables; an invalid or negative value falls back to `8` rather than disabling the wait. Fail-open and never raises — on timeout the turn answers with the inventory it has. Runs inside the turn, which holds a `GRID_MAX_ACTIVE_TURNS` lease for its duration; lower or disable it on a deployment where chat admission is the scarce resource. |
