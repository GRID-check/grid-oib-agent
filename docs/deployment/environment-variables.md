...
| `BIM_MAX_IFC_BYTES` | No | `262144000` (250 MB) | **Both** the upload ceiling for a `.ifc` and the largest file the BFF will extract — one number, so the browser cannot accept a model the parser will refuse. Deliberately independent of `FILE_UPLOAD_MAX_SIZE_MB` (100 MB), which is sized for documents: an Einreichung model is routinely 50–500 MB, and measuring one against the document limit rejected it for being a large *file*, with a message that said nothing about IFC. Parsing runs in the Node process and allocates several times the file's own size, so raise this only alongside moving extraction out of the request process (ADR-0045). Note the batch total-size limit is floored at this value, so one legal model cannot fail a limit it could never satisfy. |
