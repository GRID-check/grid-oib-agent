...
| `GRID_CONVERSATION_BUS` | No | `0` (off) | Enable the Dragonfly pub/sub conversation bus (ADR-0028) so the chat tier is fully stateless — any replica serves any conversation's WebSocket (owner publishes stream frames + HITL prompts; the socket-holding relay subscribes; HITL answers round-trip on `conv:<id>:input`). Requires `REDIS_URL`. **Off by default:** with it unset the bus uses an in-process transport (single-replica behavior is byte-identical). Enable only after a live cross-replica validation pass; then remove conversation affinity from the frontend so sockets spread independently of turn ownership. |
