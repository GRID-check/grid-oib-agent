...
| `GRID_NORMS_DIR` | No | `configs/norms` | Root of the per-country norm registry (ADR-0025): `configs/norms/<country>/registry.yml` files are globbed and merged. The registry is the typed spine for every legal document the system knows (rank, role, editions, relations, applicability) — it feeds the `ris_search` short-circuit, the `ris_catalog_lookup` tool (incl. relation annotations), and the lane-rendered block in the researcher prompts. Missing/invalid files disable registry features with a warning; live RIS search is unaffected (fail-open). RIS pointers are re-verified and merged via `scripts/build_ris_catalog.py` (live OGD-RIS check, never clobbers curated fields). |
| `GRID_RIS_CACHE_TTL_DAYS` | No | `7` | Days a fetched RIS full text in the persistent `ris_knowledge` cache (`data/ris_cache`, ADR-0025 Phase 3) is served without re-validation. Past the TTL, `ris_fetch_document` re-validates the Fassung date cheaply (first ~4 KB of the page) and only re-fetches when a Novelle actually landed; network failure during re-validation serves the stale copy rather than failing the tool. `0`/invalid values fall back to `7`. |
| `RIS_CATALOG_PATH` | No | — (deprecated) | **Deprecated (ADR-0025).** Former path of the flat curated RIS index (`configs/ris_catalog.yml`, retired). When set, the backward-compat shim `aiq_agent.common.ris_catalog` still loads that legacy file with a `DeprecationWarning`; unset, the catalog view is derived from the norm registry (`GRID_NORMS_DIR`). Migrate any override to a `configs/norms/<country>/registry.yml` entry. |
...
| `GRID_ORG_ARCHIV_ENABLED` | No | `false` | Dark-launch gate for the org-wide document **Archiv** (ADR-0024) while `GRID_ENFORCE_FEATURE_FLAGS` is off: `true` shows the `/app/archiv` page + nav entry, enables the `/api/archiv/*` routes, and injects the org's `archiv_<orgId>` collection into every project's retrieval scope. With enforcement on, the per-org `organization-archiv` WorkOS flag controls it instead. Reads are open to any org member; uploads/deletes require the `org:archiv:manage` permission. Frontend service. |
