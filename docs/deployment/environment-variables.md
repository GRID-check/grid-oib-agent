...
| `GRID_WORKFLOWS_ENABLED` | No | `false` | Dark-launch gate while `GRID_ENFORCE_FEATURE_FLAGS` is off: `true` shows the Workflows tab and enables the BFF workflow routes. With enforcement on, the per-org `workflows` WorkOS flag controls the UI/API instead. Frontend service. |
| `GRID_SKILLS_ENABLED` | No | `false` | Start gate for the skill-scheduler service container while `GRID_ENFORCE_FEATURE_FLAGS` is off — when neither this nor flag enforcement is on, the scheduler exits cleanly at boot. Frontend skill-scheduler service. |
| `GRID_SKILL_SCHEDULER_POLL_MS` | No | `30000` | Scheduler tick interval. Each tick claims due schedules (`FOR UPDATE SKIP LOCKED`), advances `next_run_at`, then fires them through the BFF internal endpoint. The deprecated `GRID_WORKFLOW_SCHEDULER_POLL_MS` name is still read as a fallback for one release (with a warning). skill-scheduler service. |
| `GRID_SKILL_SCHEDULER_BATCH` | No | `20` | Max due schedules claimed per tick. Deprecated fallback: `GRID_WORKFLOW_SCHEDULER_BATCH`. skill-scheduler service. |
...
| `GRID_SKILL_RUNS_RETENTION_DAYS` | No | `90` | Run-history retention; the scheduler prunes older `skill_runs` rows. Deprecated fallback: `GRID_WORKFLOW_RUNS_RETENTION_DAYS`. skill-scheduler service. |
