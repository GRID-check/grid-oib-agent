...
| `AIQ_VLM_MODEL` | No | `google/gemma-4-31b-it` | VLM model for vision-language tasks (image captioning during ingestion). |
...
| `AIQ_RERANKER_PROVIDER` | No | `none` | Cross-encoder reranking for `knowledge_search`: `none` \| `openrouter`. The env value is the default the config's `reranker_provider` field falls back to; the shipped config sets `openrouter`, which makes the cross-encoder primary and the `rerank_llm` judge the fallback. `none` hands reranking back to the judge. |
| `AIQ_RERANKER_MODEL` | No | `cohere/rerank-v3.5` | Reranking model id on OpenRouter. The corpus is German, so a multilingual reranker is not optional. |
| `AIQ_RERANKER_BASE_URL` | No | `https://openrouter.ai/api/v1` | Override the host (a gateway, a self-hosted reranker speaking the same request and response shape, a test double). |
| `AIQ_RERANKER_API_KEY` | No | `OPENROUTER_API_KEY`, then provider inference from the base URL | Explicit reranker key override. Resolved through the shared resolver like the embeddings key; when nothing resolves the cross-encoder is not built and the judge reranks. |
| `AIQ_RERANKER_TIMEOUT_SECONDS` | No | `10` | Per-request bound on the cross-encoder call. A cross-encoder answers in ~100ms; a provider that hangs costs this much before the judge fallback, so keep it well under the judge's own 25s. |
