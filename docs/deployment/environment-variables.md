...
| `FILE_UPLOAD_MAX_SIZE_MB` | No | `100` | Maximum size in MB, applied both per individual file and to the total of one upload. What the TOTAL is measured over depends on the surface — see `FILE_UPLOAD_MAX_FILE_COUNT` below, which is scoped identically. A `.ifc` building model is measured against `BIM_MAX_IFC_BYTES` (default 250 MB) instead. |
...
| `FILE_UPLOAD_MAX_FILE_COUNT` | No | `10` | Maximum number of files one upload may carry. **Scoped per surface**: for a **chat session** the cap is cumulative over the conversation's existing attachments (a session holds at most this many files in total — the attachment set is a bounded, throwaway working set); for a **durable store** (a project's Dateiablage, the org Archiv) it applies to the **incoming upload only**, and the documents already stored do not count against it. Applying the session's cumulative arithmetic to a durable corpus made `FILE_UPLOAD_MAX_FILE_COUNT` a permanent per-project document ceiling (issue #432: `Would have 11 files. Only 0 more allowed (10 max).`), and `FILE_UPLOAD_MAX_SIZE_MB` a permanent per-project byte ceiling a few files later. Neither was ever a real resource control on a durable store — `POST /api/documents/upload` takes ONE file per request, so a client-side batch total bounds nothing. What bounds a durable store is `GRID_DEFAULT_STORAGE_QUOTA_BYTES` / the org's Storage setting, enforced server-side inside the transaction that records the document (ADR-0042). |
