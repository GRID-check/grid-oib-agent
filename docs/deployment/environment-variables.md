...
| `AIQ_EMBED_TIMEOUT_SECONDS` | No | `60` | Per-request timeout on the embeddings client (query and ingest alike), with two retries. The client's own defaults were 120s and five retries, so a hung embeddings endpoint could hold a chat turn for ten minutes; now the retrieval fails open after at most three bounded attempts like every other retrieval step. |
