...
| `GRID_LANDING_URL` | No | — | Base URL of the public landing site (Astro microservice, `frontends/web`) that replaces the retired in-app marketing page. The signed-out root redirect (`/`) points here; unset falls back to the WorkOS sign-in URL. Set by the Kubernetes deployment from `ingress.webDomain`. |
