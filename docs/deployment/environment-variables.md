...
| `GRID_STAGE_MAX_CONCURRENCY` | No | `4` | Maximum post-answer stage handlers running concurrently per backend process, **shared across all stages** (memory reflection today). Stages share the event loop with live chat turns; this bounds their background LLM traffic. Replaces `MEMORY_REFLECTION_MAX_CONCURRENCY`, which the stage primitive superseded — see `docs/architecture/post-answer-stages.md` §7.9. |
