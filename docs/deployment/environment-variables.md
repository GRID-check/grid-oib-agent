...
| `post-answer-follow-ups` *(WorkOS feature flag, not env)* | No | off | Runtime on/off switch for the post-answer follow-up-questions stage **when `GRID_ENFORCE_FEATURE_FLAGS=true`**: evaluated per organization **per turn**, served to the backend by `GET /api/internal/stages`; fail-closed. Without enforcement this flag is ignored and `GRID_STAGE_FOLLOW_UPS_ENABLED` decides instead. Switching it on for an org gives that org's readers the follow-up rail below the answer; switching it off stops the frames on the next turn, with no deploy and no effect on the answer either way. See `docs/architecture/post-answer-stages.md` §10.
| `GRID_STAGE_FOLLOW_UPS_ENABLED` | No | `false` | Runtime on/off switch for the follow-up-questions stage **when `GRID_ENFORCE_FEATURE_FLAGS` is off** (the default). New stages default OFF: this one costs an LLM call per substantive answer. Frontend service. The backend additionally no-ops when the workflow config sets no `follow_ups_llm` (the capability bit).
