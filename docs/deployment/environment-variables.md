...
| `GRID_IFC_MODELS_ENABLED` | No | `false` | Dark-launch gate for IFC/BIM models (ADR-0045) while `GRID_ENFORCE_FEATURE_FLAGS` is off. `true` puts `.ifc` in the upload accept-list (client and the server-side allow-list), shows the Model nav entry and `/model` page, and enables every `/api/**/bim/*` route plus the agent's `ifc_query` tool. With enforcement on, the per-org `ifc-models` WorkOS flag controls them instead. **Default-off deliberately, not fail-open like ordinary flags:** the feature renders OIB compliance verdicts, and a checker that prints `erfüllt` makes a claim an architect may act on — so it must not switch itself on in an environment whose operator has not chosen it. No paired capability variable: extraction runs in the BFF process with `@ifc-lite/parser` and the viewer runs in the browser, so there is no infrastructure dependency to gate on; the viewer's WebGPU requirement is a per-browser fact detected at render time, which degrades to the data explorer. Frontend service. On Kubernetes it is set from the Pulumi stack key `grid-oib:ifcModelsEnabled` (see `deploy/pulumi/README.md`), not by hand. |
...
| `BIM_MAX_IFC_BYTES` | No | `262144000` (250 MB) | Largest `.ifc` the BFF will extract, independent of the upload size limit. Parsing runs in the Node process, so a 600 MB federated model would hold a worker for minutes and allocate several times its own size; above this the model is refused with a readable reason rather than taking the process down (ADR-0045). Raise only alongside moving extraction out of the request process. |
