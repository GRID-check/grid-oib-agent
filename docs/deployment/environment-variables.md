...
| `SEAWEED_TENANT_ADMIN_ACCESS_KEY` | No | `grid-tenant-admin` in both compose stacks; falls back to `SEAWEED_ACCESS_KEY` when unset | Access key of the identity allowed to create and drop tenant buckets. A second credential rather than a second client, because SeaweedFS's `Admin:<bucket>` authorises CreateBucket and DeleteBucket together and cannot express one without the other — keeping bucket lifecycle off the upload path means keeping it on a key the upload path does not hold. On Kubernetes set via `grid-oib:seaweedfsTenantAdminAccessKey` (default `grid-tenant-admin`). Frontend service only: the purger gets the two naming variables above so it can reconstruct which buckets an organization's objects are in, but deliberately not this credential — an unattended queue worker is the last process that should be able to drop a bucket. |
