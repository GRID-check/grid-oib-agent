...
| `SEAWEED_PER_ORG_BUCKETS` | No | `false` | `true` writes each organization's objects into its own bucket instead of a key prefix inside `SEAWEED_BUCKET` (ADR-0043), which makes erasing a tenant one `DeleteBucket` rather than a paginated sweep that can half-finish. Only the literal string `true` enables it. Not a cutover: the bucket is recorded on each row (`documents.storage_bucket`, NULL = `SEAWEED_BUCKET`), so flipping it changes where the **next** object is written and leaves every existing object readable where it is. On Kubernetes set via `grid-oib:seaweedfsPerOrgBuckets`. Frontend + purger services. |
| `SEAWEED_TENANT_BUCKET_PREFIX` | No | `grid-org-` | Leading segment of a tenant bucket name; the rest is the slugged organization id plus a truncated SHA-256 of the original id (`src/lib/storage/tenant-bucket.js`, shared by the BFF and the purger so the process that ERASES a tenant cannot disagree with the one that wrote it). Also the string the tenant S3 grants are wildcarded on (`Read:<prefix>*`), so it must not be a prefix of any platform bucket name. Changing it after objects exist does not move them — their rows still name the old bucket — but new writes go elsewhere. Frontend + purger services. |
| `SEAWEED_TENANT_ADMIN_ACCESS_KEY` | No | falls back to `SEAWEED_ACCESS_KEY` | Access key of the identity allowed to create and drop tenant buckets. A second credential rather than a second client, because SeaweedFS's `Admin:<bucket>` authorises CreateBucket and DeleteBucket together and cannot express one without the other — keeping bucket lifecycle off the upload path means keeping it on a key the upload path does not hold. On Kubernetes set via `grid-oib:seaweedfsTenantAdminAccessKey` (default `grid-tenant-admin`). Frontend service only: the purger gets the two naming variables above so it can reconstruct which buckets an organization's objects are in, but deliberately not this credential — an unattended queue worker is the last process that should be able to drop a bucket. |
| `SEAWEED_TENANT_ADMIN_SECRET_KEY` | No | falls back to `SEAWEED_SECRET_KEY` | Secret key for the identity above. Required in practice wherever `SEAWEED_PER_ORG_BUCKETS=true`; the fallback exists so a deployment with per-org buckets off — where nothing calls the bucket-admin client — needs no extra credential. On Kubernetes it ships in the `grid-secrets` Secret from `grid-oib:seaweedfsTenantAdminSecretKey`, which the stack requires when per-org buckets are on. Frontend service only (see above). |
