...
| `AIQ_DEEP_CHECKPOINT_DB` | No | `./deep_research_checkpoints.db` (reference config default) | `./deep_research_checkpoints.db` | Optional durable LangGraph checkpointing for **async deep-research jobs** (backlog T3-8, 2026-07-16) — separate from `AIQ_CHECKPOINT_DB` above, which only covers the sync chat graph. Sets `deep_research_agent.checkpoint_db`; a worker crash no longer loses execution state (`thread_id = job_id`, `durability="async"`), but resume is manual-resubmit-based, not automatic. Unset at the config-key level (`checkpoint_db: null`) disables durability entirely for that config. |
