...
| `AIQ_EXTRACT_IMAGES` | No | `false` (code default; both deployments set `true`) | Enable extraction of **embedded raster images** (image XObjects) from PDFs for VLM captioning, so a photo inside a plan set is findable and its page can be shown with `view_knowledge_image`. Does NOT capture vector CAD drawings — see `AIQ_RENDER_VISUAL_PAGES`. |
