...
| `GRID_ENFORCE_FEATURE_FLAGS` | No | `false` | `true` enforces WorkOS feature flags (`runtime-model-config`, `deep-research` — registry: `lib/authz/feature-flags.ts`). Turn on only after the flags exist in WorkOS and orgs are targeted; sessions without the JWT `feature_flags` claim then fail closed until re-login. Frontend + skill-scheduler services. It reached only the **scheduler** in compose until 2026-09-10, so `enforcementOn()` in the BFF read an unset variable however the operator had configured it, and the two containers could disagree about whether flags decided anything; both compose files now pass it to the frontend as well. Its effective value is on the `[boot]` line, so a deployment no longer has to be trusted about it. |
