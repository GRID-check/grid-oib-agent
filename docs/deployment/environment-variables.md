...
| `GRID_AVAILABLE_DOCUMENTS_MAX` | No | `50` | Top-N cap on the project document summaries injected into every chat turn's prompt (`available_documents`). The summary `SELECT` is unbounded and every turn renders the list into ~5 templates, so per-turn LLM cost grew linearly with the corpus. The aggregated set is sorted by filename (stable across turns → better prompt caching) then capped. `0`/negative disables the cap. |
| `GRID_CHAT_CHECKPOINT_RETENTION_SECONDS` | No | `1209600` (14d) | Idle-thread retention for the chat checkpoint store (`AIQ_CHECKPOINT_DB`). A leader-locked hourly reaper drops whole conversation threads whose newest LangGraph checkpoint `ts` is older than this, so the checkpoint tables stay bounded (chat has no terminal event). `0`/invalid falls back to the default; only meaningful on Postgres. |
| `GRID_JOB_INFO_DELETE_GRACE_SECONDS` | No | `604800` (7d) | In `db` execution mode (ADR-0021, no Dask), the leader-locked cleanup marks terminal `job_info` rows expired past their per-row expiry and hard-deletes rows (with their `job_access`/`job_events`) once `updated_at` is older than this grace. Always keeps the single most-recent finished job. `0`/invalid falls back to the default. |
| `GRID_CHECKPOINT_POOL_MIN_SIZE` | No | `1` | Min connections in the Postgres LangGraph checkpointer pool (per replica, per distinct checkpoint DSN). `0`/invalid falls back to `1`. |
| `GRID_CHECKPOINT_POOL_MAX_SIZE` | No | `10` | Max connections in the Postgres LangGraph checkpointer pool (per replica, per distinct checkpoint DSN) — raised from a former hard-coded `3` that throttled the chat tier's concurrent turns. Floored to the min size; `0`/invalid falls back to `10`. |
