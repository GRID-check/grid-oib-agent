...
| `GRID_APP_RUNTIME_PASSWORD` | No | `grid_app_rw_dev` | n/a | Password `deploy/compose/init-db.sql` sets on the `grid_app_rw` role at container start. On Kubernetes the `pg-init-tables` Job sets it from the Pulumi secret `pgRuntimePassword` (which defaults to `pgAppPassword`). |
