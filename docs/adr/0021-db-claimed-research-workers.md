...
- **Status:** Accepted — implemented behind `GRID_JOB_EXECUTION=db|dask` (default `dask`). See `frontends/aiq_api/src/aiq_api/jobs/{queue,worker}.py`, the submit switch in `jobs/submit.py`, and the `agent-worker` tier in `deploy/pulumi`. The `agent-worker` tier scales horizontally (stateless, DB-claimed). The chat/web tier also scales horizontally in `db` mode (`backendReplicas`, default 2): stateless parts share Chroma/Postgres/Dragonfly, and the in-process WebSocket/HITL/task state is made safe by **conversation affinity** (the frontend pins each conversation to its owning replica — ADR-0028). See docs/architecture/scaling-review-2026-07-phase2.md and kubernetes.md §6.4.
- **Date:** 2026-07-09
