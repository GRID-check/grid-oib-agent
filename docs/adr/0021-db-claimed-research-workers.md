...
- **Status:** Accepted — implemented behind `GRID_JOB_EXECUTION=db|dask` (default `dask`). See `frontends/aiq_api/src/aiq_api/jobs/{queue,worker}.py`, the submit switch in `jobs/submit.py`, and the `agent-worker` tier in `deploy/pulumi`. The `agent-worker` tier scales horizontally (stateless, DB-claimed). The chat/web tier's stateless parts are replica-safe (shared Chroma/Postgres/Dragonfly, ingest-status store, leader-locked background loops), but the interactive WebSocket path is not yet (in-process WS/HITL/task state, no conversation-affinity), so `backendReplicas` defaults to 1 — see docs/architecture/scaling-review-2026-07-phase2.md and kubernetes.md §6.4.
- **Date:** 2026-07-09
