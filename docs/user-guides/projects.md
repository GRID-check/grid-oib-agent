...
**All projects** — everything else follows as a dense list, most recent first. Each row carries the project's initials, its name and brief, the **Active** status chip, the document count and the timestamp, plus the same settings gear. Rows lift onto the card surface as you point at them; on narrow screens the chips and counts drop away and the timestamp stays, because that is what you choose a row by. Fewer than four projects and there is no list at all — the cards are the whole page.
