...
| **3** (~1 wk) | `ris_knowledge` persistent cache **with the §4.4 freshness policy**, `corpus/ris.py` § chunking, stratum labels, scope fan-out | Stale-copy TTL re-validation proven by test (simulated Novelle → re-fetch); results show rank/role labels |
| **4** (~2 wk) | Applicability rules in registry (per the pinned DSL), Python resolver, TS codegen + parity test, new intake triggers, compliance Stage 1 integration, UI parity | Overview UI and backend prompts produce identical verdicts on fixture profiles |
