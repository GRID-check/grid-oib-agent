...
> **Archived.** Frozen record of a finished loop, moved here from `run_log.md`
...
- Created `backlog.md` (repo root; seeded from this session's deep diagnosis + known-issues docs) and this log.
...
## Cycle 20 — T3-6 · uniform 403 for authz failures across API routes — `lib/auth/require-auth.ts` (+ spec) + 19 route handlers

- **Wrong & why:** 19 API route files/~20 handlers let authorization errors escape as opaque HTTP 500s — 16 called `requireAuthorizedSession()` with no try/catch (so `NoOrganizationError` leaked to Next.js), 3 had a try/catch that always returned 500 without classifying authz. Resource-level authz throws (`requireProjectAccess`, `role !== 'admin'`) inside the bare routes had the same fate. Sonnet inventory corrected the backlog's "~17 bare" to 16 bare + 3 mis-wrapped, and confirmed 9 routes already handle it via a hand-copied `isAuthzError` idiom (no shared abstraction).
- **User mid-cycle question:** "why not do this in middleware with a path regex instead of every route?" Investigated `src/proxy.ts` (the AuthKit `authkitMiddleware`). Assessment: middleware only gates *authentication*, not org membership; an org-gate there (a) needs an allowlist for org-less endpoints (org creation `POST /api/organizations`, onboarding, `/api/internal/*`, `/api/auth/*`) or it breaks onboarding, (b) runs on Edge and must re-read the session, and (c) fundamentally can't do per-resource authz thrown *during* handling — which the bare routes ALSO leak as 500. So middleware is a coarse front gate, not a replacement. User chose the route-level fix.
- **Change (Opus decision):** shared `authzErrorResponse(error): NextResponse | null` in server-only `require-auth.ts` (uses `isAuthzError`; helper NOT put in the client-safe `auth-utils.ts` to avoid pulling `next/server` client-side). Applied a uniform, signature-preserving try/catch per handler (delegated the 19-file mechanical edit to an Opus subagent; TYPE 1 = wrap body, TYPE 2 = prepend delegation to the existing catch). Chose signature-preserving over a `withAuthorizedApi` HOF to avoid changing 28 export signatures blind overnight.
- **Verified:** `git diff -w --numstat` proved purely-additive wrapping (exactly 1 removed line/file = the import rewrite; zero body lines dropped); spot-checked a 3-handler TYPE-1 and both TYPE-2 files; grid-tsc Docker `tsc --noEmit` exit 0; vitest 29/29 across `require-auth` (incl. 5 new helper tests) + folders/overview/preview route specs (confirmed no existing 500-expectation flipped to 403).
- **Pipeline:** Sonnet — full route inventory + failure-class taxonomy + test map. Opus (orchestrator) — architecture assessment for the user's middleware question, helper design + tests, delegated mechanical application to an Opus subagent, then hard-verified (diff/tsc/specs).

## CLOSING SUMMARY (run 2 — re-entry on branch `fix/agent-e2e-memory-cards`, model Opus 4.8)

- **Cycles this re-entry:** 17–20 (4 cycles). **Commits:** 8 (4 code + 4 bookkeeping). Prior run (cycles 0–16, branch `feature/applicable-oib-standards`) summarized above.
- **By tier this re-entry:** Security/PIN-1 — DEL-F4 (stranded-'purging' reaper), DEL-F9 (constant-time internal-token compare), DEL-F5/F8 (surface last_error), closing PIN-1's entire DEL-F1–F10 code list; DEL-F10 (db.js SQL tests). Bugs/Stability — T3-6 (uniform 403 for authz failures across 19 API routes + shared helper).
- **Verification:** host node_modules install is incomplete (no vitest bin), so the `grid-tsc` Docker image (`npm ci`) is the only working JS/TS runner; backend via `.venv` py_compile/ruff/pytest. All changes verified green (purger 14/14, maintenance auth 6/6, T3-6 tsc 0 + 29/29).
- **#1 thing for a human first:** the T3-6 change touches 19 route files — mechanically uniform and tsc+spec-verified, but only 4 of those routes have specs; a quick runtime smoke of a no-org session hitting a couple of these endpoints (expect 403 JSON, not 500/HTML) would fully close it.
- **Security items still FLAGGED, not fixable in-loop:** T1-2 rotate live secrets in `deploy/.env`; T1-3 set a real `GRID_INTERNAL_API_TOKEN` (dev default now refused outside dev). Optional: middleware org-gate as defense-in-depth (needs allowlist + runtime pass); migrate the 9 GUARDED routes to `authzErrorResponse` to de-dup.
- **Branch note:** loop commits sit alongside the user's own in-flight commits on this branch; uncommitted WIP (chat_researcher intent, auth validators, cards models, project_memory, json_utils) was deliberately left untouched throughout.
