...
| `task: command not found`, in a checkout that is otherwise fine | Nothing in this repo installs go-task, and `task setup` cannot install the tool that runs it | `scripts/agent-preflight.sh` bootstraps a pinned `@go-task/cli` into `.tools/` and runs setup. `eval "$(scripts/agent-preflight.sh --print-env)"` puts it on PATH for the current shell |
| Claude blocks every edit with "this checkout has not been set up" | The `PreToolUse` guard: `task setup` never completed here, so nothing you write can be linted, typechecked or tested | Run `scripts/agent-preflight.sh` and read what it reports. It is the session-start hook having failed, so fix that failure rather than working around the guard. Last resort for a checkout that cannot be provisioned: `GRID_AGENT_PREFLIGHT=skip`. See [agent-preflight.md](agent-preflight.md) |
| Setup ran successfully yesterday and the preflight says it is stale today | A pull moved a lockfile. The stamp hashes `uv.lock`, `apm.lock.yaml`, `bun.lock` and the three `package-lock.json` files | Re-run `scripts/agent-preflight.sh`. This is the mechanised form of `AGENTS.md`'s "after any pull that moves a lockfile" |
