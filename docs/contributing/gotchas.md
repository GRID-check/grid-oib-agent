...
| A server-side render throws `Minified React error #31` — an object "with keys {$$typeof, type, key, ref, props}" — and every PDF the app files fails | Two Reacts. Next vendors its own React and aliases every `react` import in the code it bundles to it; the packages it refuses to bundle (`serverExternalPackages`, which includes `@react-pdf/renderer` by Next's own default) resolve the `react` in `node_modules` instead. `@react-pdf/reconciler` reads that copy's version to choose a reconciler, and React 18's and 19's disagree about an element's `$$typeof`, so it recognises nothing the app hands it | Keep `react` and `react-dom` in `frontends/ui/package.json` on the major Next vendors. `src/lib/pdf/vendored-react-parity.spec.ts` renders through both copies and fails when they drift; ordinary specs cannot see this, because under Vitest both sides of a render resolve to the same copy |
