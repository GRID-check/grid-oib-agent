...
| Per-turn LLM cost grows with the size of a project | Something builds a prompt block by iterating rows — documents, bindings, memories — with no bound. It is paid on every turn, including chit-chat | Carry the shape, not the list: one line per category with a count, and a tool to fetch the members. `_available_documents_limit` and `buildDocumentRolesSection` are the two worked examples |
