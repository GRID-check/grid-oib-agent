...
| Freigabe und Fassungen, rendered | `frontends/ui/src/features/documents/components/document-lifecycle-panel.tsx`; mounted in `frontends/ui/src/features/documents/components/file-preview-pane.tsx` | same | ADR-0054 |
...
| Reading one run — where its message is and what its ledger says | `frontends/ui/src/app/api/projects/[id]/runs/[runId]/route.ts` — `getRunView` (`project:view`); the browser's typed client is `frontends/ui/src/lib/runs/run-view-client.ts` — `fetchRunView` | same | ADR-0062 |
| The research plan a deep research waits on: proposed by the clarifier, edited, held and started on the run block, claimed by the worker (ADR-0068) | `frontends/ui/src/lib/plans/plan-types.ts` (the contract), `frontends/ui/src/lib/plans/service.ts` (`proposePlannedRun`, `editPlan`, `holdPlan`, `startPlan`, `claimPlanStart`); `src/aiq_agent/common/research_plan.py` (mirror, `render_plan_context`); `src/aiq_agent/agents/piloti/clarify.py` (`draft_plan`); `frontends/aiq_api/src/aiq_api/jobs/plan_start.py` (the wait); `frontends/ui/src/features/runs/components/RunPlan.tsx`, `frontends/ui/src/features/runs/components/PlanDialog.tsx` | [`docs/architecture/deep-research-report.md`](deep-research-report.md#the-rechercheplan-before-the-run) | ADR-0068 |
| The Unterlagen a run is told to read (Grundlage) or refuse (Ausgeschlossen), from the plan to the source registry | `src/aiq_agent/common/plan_documents.py` — `PlanDocuments`, `documents_from_plan`, `unread_grundlage`; its mirror `frontends/ui/src/lib/runs/plan-documents.ts`; the plan's document step `frontends/ui/src/features/runs/components/PlanUnterlagen.tsx`, which opens the document picker `frontends/ui/src/features/documents/components/document-picker/DocumentPickerDialog.tsx`; the refusal `src/aiq_agent/agents/deep_researcher/custom_middleware.py` — `SourceRegistryMiddleware.is_excluded`; a live addition `frontends/aiq_api/src/aiq_api/routes/jobs.py` (`/documents`) → `src/aiq_agent/agents/deep_researcher/control.py` | [`docs/architecture/deep-research-report.md`](deep-research-report.md#unterlagen-what-the-run-reads-and-what-it-may-not) | ADR-0062 |
| The run's title on its message (`metadata.run_title`), and how it reaches the block's header | `frontends/ui/src/lib/runs/service.ts` — `createRunMessage` takes `title` from `submitAgentRun` (`frontends/ui/src/lib/jobs/service.ts`, the definition's title) and `dispatchRun` (`frontends/ui/src/lib/tasks/delegation.ts`, the task's); `frontends/ui/src/lib/runs/run-ledger.ts` — `sanitizeRunTitle`; read back by `frontends/ui/src/features/chat/lib/server-message-mapper.ts` as `message.runTitle` | same | ADR-0062 |
| How a run's block in the thread stays live — the stored ledger, replaced by every `run.ledger` snapshot from the run's own stream, one subscription per block | `frontends/ui/src/features/runs/hooks/use-run-ledger.ts` — `useRunLedger`; the `run.ledger` event lands in `frontends/ui/src/adapters/api/deep-research-client.ts` — `onLedger` | [`docs/api/websocket-protocol.md`](../api/websocket-protocol.md#run-event-streams) | ADR-0062 |
| Where the thread decides a message is a run and renders the block, with the report beneath it once there is one | `frontends/ui/src/features/layout/components/ChatArea.tsx` — `MessageRendererComponent` (an assistant message with `runLedger`) -> `frontends/ui/src/features/runs/components/RunBlockMessage.tsx` -> `RunBlock` | same | ADR-0062 |
| What a running task is doing, as the one line under its card in the Aufträge list (`runSummary` on `TaskWireRow`) | `frontends/ui/src/lib/tasks/list-projection.ts` — `runSummaryOf`, `loadRunSummaries` (one query bounded to the page, active rows only, via `frontends/ui/src/lib/conversations/repository.ts` — `listRunLedgersByMessageIds`); shown by `frontends/ui/src/features/tasks/components/task-card.tsx` | same | ADR-0062 |
| The whole run behind one Aufträge row, when the drawer is opened on it | `frontends/ui/src/features/tasks/hooks/use-task-run.ts` — one read of `GET /api/projects/[id]/runs/[runId]`, repeated only when the panel's poll moves the row; rendered as the real `RunBlock` by `frontends/ui/src/features/tasks/components/task-detail.tsx` | same | ADR-0062, `docs/design/run-block.md` |
| Whether the skill being written has been checked, and what the reviewer found about each field | `frontends/ui/src/features/skills/hooks/use-skill-review.ts` — the verdict, its staleness and the save's gate, held above all three builder steps; rendered under each field by `frontends/ui/src/features/skills/components/SkillFindingList.tsx` | same | `docs/architecture/agent-skills.md` |
| Which thread a run lands in, and where its message is minted | `frontends/ui/src/lib/jobs/service.ts` — `submitAgentRun` (takes the run id and the commissioning conversation), `fireJob`, `createTaskThread`; `frontends/ui/src/lib/tasks/delegation.ts` — `dispatchRun` | [`docs/database/schema.md`](../database/schema.md) | ADR-0062, ADR-0051 |
| The ONE thread a standing task owns, and how a reader tells it from the old per-fire conversations | `frontends/ui/src/lib/tasks/task-thread.ts` — `taskThreadConversationId`, `isTaskThread`; the sessions list applies it through `frontends/ui/src/features/chat/lib/project-scope.ts` — `isHiddenJobConversation` | same | ADR-0062 |
| How a chat question becomes a run instead of an answer (the escalation) | `src/aiq_agent/turn/commission.py` — `commission_research_run`, which POSTs `op: 'research'` to the one internal task door; the decision to use it at all is `src/aiq_agent/turn/dispatch.py` — `build_run_commissioner`; the node is `src/aiq_agent/agents/piloti/conversation.py` — `_commission_run` | [`docs/api/bff-routes.md`](../api/bff-routes.md) | ADR-0062, ADR-0055 |
| Where that run is written, with no definition behind it | `frontends/ui/src/lib/tasks/delegation.ts` — `commissionResearchRun` and the shared gate `COMMISSION_PERMISSIONS`; the door is `frontends/ui/src/app/api/internal/tasks/route.ts` | same | ADR-0062, ADR-0051 |
| How the reader's open thread gets hold of the run's message without a reload | `frontends/ui/src/features/chat/lib/commissioned-run.ts` — `fetchRunMessage`; the store adopts it with the SERVER's id (`adoptRunMessage`, `frontends/ui/src/features/chat/stores/messages-store.ts`) | — | ADR-0062 |
| Stopping a run from its block | `frontends/ui/src/app/api/projects/[id]/runs/[runId]/cancel/route.ts` -> `frontends/ui/src/lib/runs/service.ts` — `cancelRun` -> `frontends/ui/src/lib/jobs/backend-client.ts` — `cancelBackendJob`; the control is `RunBlock`'s „Abbrechen" behind `frontends/ui/src/components/ui/confirm-dialog.tsx` | [`docs/api/bff-routes.md`](../api/bff-routes.md) | ADR-0062, ADR-0055 |
| The order the block's moves play in when a run lands | `frontends/ui/src/features/runs/lib/choreography.ts` — `landingDelays`, summed from the motion kit's own constants (`frontends/ui/src/components/motion/index.tsx`) | [`docs/design/grid-design-language.md`](../design/grid-design-language.md) | — |
| Where a finished run's report is written, from the one id the worker holds | `frontends/ui/src/app/api/internal/runs/by-job/[backendJobId]/report/route.ts` -> `frontends/ui/src/lib/runs/service.ts` — `writeRunReport`, `findRunMessageByBackendJobId`; the caller is `frontends/aiq_api/src/aiq_api/jobs/conversation_output.py` — `write_job_turn`, `write_job_notice` | [`docs/api/bff-routes.md`](../api/bff-routes.md) | ADR-0062, ADR-0055 |
