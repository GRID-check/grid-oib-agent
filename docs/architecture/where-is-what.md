...
| The Unterlagen a run is told to read (Grundlage) or refuse (Ausgeschlossen), from the plan to the source registry | `src/aiq_agent/common/plan_documents.py` — `PlanDocuments`, `documents_from_plan`, `unread_grundlage`; its mirror `frontends/ui/src/lib/runs/plan-documents.ts`; the picker `frontends/ui/src/features/runs/components/UnterlagenDialog.tsx`; the refusal `src/aiq_agent/agents/deep_researcher/custom_middleware.py` — `SourceRegistryMiddleware.is_excluded`; a live addition `frontends/aiq_api/src/aiq_api/routes/jobs.py` (`/documents`) → `src/aiq_agent/agents/deep_researcher/control.py` | [`docs/architecture/deep-research-report.md`](deep-research-report.md#unterlagen-what-the-run-reads-and-what-it-may-not) | ADR-0062 |
