...
| The research plan a deep research waits on: proposed by the clarifier, edited, held and started on the run block, claimed by the worker (ADR-0065) | `frontends/ui/src/lib/plans/plan-types.ts` (the contract), `frontends/ui/src/lib/plans/service.ts` (`proposePlannedRun`, `editPlan`, `holdPlan`, `startPlan`, `claimPlanStart`); `src/aiq_agent/common/research_plan.py` (mirror, `render_plan_context`); `src/aiq_agent/agents/piloti/clarify.py` (`draft_plan`); `frontends/aiq_api/src/aiq_api/jobs/plan_start.py` (the wait); `frontends/ui/src/features/runs/components/RunPlan.tsx`, `frontends/ui/src/features/runs/components/PlanDialog.tsx` | [`docs/architecture/deep-research-report.md`](deep-research-report.md#the-rechercheplan-before-the-run) | ADR-0065 |
