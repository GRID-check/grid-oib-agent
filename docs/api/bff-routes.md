...
| `POST` | `/api/skills` | org:skills:manage | Author a skill. Validates the SKILL.md name/description rules and the reserved `grid-execution` value; `clonedFrom` records a platform clone. | `{ name, description, body, metadata?, clonedFrom?, enabled? }` | `{ skill }` (201) |
...
Project skill schedules (read = `project:view`, mutate/run = `project:skills:manage` via `requireProjectAccess`):

| Method | Path | Auth | Description | Request Body / Params | Response |
|--------|------|------|-------------|-----------------------|----------|
| `GET` | `/api/projects/{id}/skill-schedules` | project:view | List the project's schedules (each carrying its skill snapshot). | — | `{ schedules }` |
| `POST` | `/api/projects/{id}/skill-schedules` | project:skills:manage | Create. Resolves the named skill (org row first, builtin fallback; unknown → 404), snapshots it, denormalizes `execution`, validates cron (+ min interval, IANA timezone) and computes `next_run_at`. A cron on a `grid-schedulable: "false"` skill is refused. | `{ name, skillName, dataSources?, enabled?, scheduleCron?, scheduleTimezone? }` | `{ schedule }` (201) |
| `GET` | `/api/projects/{id}/skill-schedules/{scheduleId}` | project:view | Get one schedule. | — | `{ schedule }` |
| `PATCH` | `/api/projects/{id}/skill-schedules/{scheduleId}` | project:skills:manage | Update; re-resolves the snapshot and recomputes `next_run_at`. | partial create body | `{ schedule }` |
| `DELETE` | `/api/projects/{id}/skill-schedules/{scheduleId}` | project:skills:manage | Delete the schedule (runs cascade). | — | `{ deleted: true }` |
| `POST` | `/api/projects/{id}/skill-schedules/{scheduleId}/run` | project:skills:manage | Manual "Run now" through the shared fire path. 409 when disabled; a backend 429 (job caps) comes back as a `skipped` run, not an error. | — | `{ run }` |
| `GET` | `/api/projects/{id}/skill-schedules/{scheduleId}/runs` | project:view | Run history, newest first. | `?limit&offset` | `{ runs }` |
...
Source: `frontends/ui/src/app/api/skills/…` and `…/api/projects/[id]/skill-schedules/…`, service in `frontends/ui/src/lib/skills/`. See `docs/architecture/agent-skills.md`.
...
| `POST` | `/api/internal/skills/fire` | `x-grid-internal-token` | Scheduler-fired skill run (`{ scheduleId }`). Re-checks `enabled` + the org's skills gate, then submits through the shared fire path (Agent Skills, ADR-0046). |
