...
| `POST` | `/api/internal/tasks` | `x-grid-internal-token` **plus a verified signed envelope** | **New 2026-09-10 (ADR-0051, ADR-0055).** The ONE route a machine may delegate work through — the `create_task` tool's only call. Op set is closed to `create`: a machine may ASK for work and may never judge it, so reviewing a task stays the session route `POST /api/projects/[id]/tasks/[taskId]/review`. Body is `{ op: 'create', projectId, kind, goal, due? }` with `kind` one of `compliance_check | einreichcheck | document | revision` (`DELEGATABLE_TASK_KINDS`); `due` is `YYYY-MM-DD` (read as the END of that day) or a full ISO instant, and an unparseable one is dropped rather than refused. Identity is the document-versions route's, through the same helper (`lib/api/internal-envelope.ts`): the verified `X-Grid-Request-Context` envelope names the acting person, `resolvePinnedRequesterSession` builds their real session, and the task's `requester_user_id` is pinned to them — so the run spends their budget and acts under their permissions. `project:edit` + `project:documents:write` on the named project, checked in that session. The tenant slot comes from the verified envelope, never from the body, and `body.projectId` must equal the envelope's `projectId` when it names one (`requireEnvelopeProject`, 400 otherwise) — a task queues a run that files as this person, so a replayed envelope pointed at a second project would be work nobody asked for, attributed to somebody who did not ask for it. Answers `{ taskId, kind, title, status, conversationId, dueAt }` with 201. |
