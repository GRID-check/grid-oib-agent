...
### The document lifecycle (ADR-0054, ADR-0055)

Every document has a version history; a version carries the editorial state. A
human upload writes version 1 `published`, born approved — the person who
uploaded it is the assertion — and a re-upload under the same name writes
version N+1 and leaves N standing as `superseded` **with its bytes intact**. An
agent filing writes a `draft` that walks the review states. The routes below are
the same for both, and they sit under the existing `/api/documents/{id}` path so
each resolves its own shelf's permission from the row (`getAccessibleDocument`).

**There is one typed client** (`frontends/ui/src/lib/documents/lifecycle-client.ts`)
and the route specs drive the real handlers through it. That is ADR-0055's rule,
not a convenience: the UI, the agent's tools, the task runner and any later
integration are equal clients of one API, and no service function is reached
from a second path.

| Method | Path | Auth | Notes | Body | Returns |
|---|---|---|---|---|---|
| `GET` | `/api/documents/{id}/versions` | Required | Every version of a document, oldest first, with the item's `lifecycle` and its `published_version_id`. `project:view` for a project document; an Archiv document is readable by any org member, a chat attachment by a conversation viewer. | — | `{ documentId, lifecycle, publishedVersionId, versions: [...] }` |
| `GET` | `/api/documents/{id}/versions/{versionId}` | Required | One version. Same posture. | — | `{ version }` |
| `POST` | `/api/documents/{id}/versions` | Required | Fork a new `draft` from the published version. `project:documents:write` (or the legacy `project:edit` umbrella) **and** `project:documents:generate`. The draft SHARES the published version's storage key until its content is replaced — copying would charge the quota twice for a file nobody has changed. At most one open version per document (`uniq_document_versions_open_per_document`): a second attempt is a `409 CONFLICT` naming the draft that exists. | `{}` | `{ version }` (201) |
| `PUT` | `/api/documents/{id}/versions/{versionId}/content` | Required | Replace an open version's bytes, **whole body**, with `ifMatch` on the stored `content_hash`. A mismatch is `409 CONFLICT`, never a last-write-wins overwrite: two turns of one conversation can both be holding the same draft. There is deliberately NO string-replacement verb — editing happens in the conversation's working directory, filing is a whole document. The bytes land under the version's own `v<n>/` key, so the published version's object is never written over. | `{ content, ifMatch }` | `{ version }` |
| `GET` | `/api/documents/{id}/versions/{versionId}/content` | Required | One version's bytes as `text/plain` — what „Öffnen“ in the version list opens, including for a superseded version. Same read posture as the version itself. The lifecycle's versioned documents are Markdown, so text is the honest projection; a binary version is opened through the document's own file route, which knows its content type. | — | the version's text |
| `POST` | `/api/documents/{id}/versions/{versionId}/submit` | Required | Hand the version to a reviewer. `project:edit` / `project:documents:write`. `reviewerUserIds` may be empty, and empty is the ORDINARY case — a Piloti draft is `Unvergeben` by construction, so `lib/documents/reviewers.ts` resolves a chain: the people named, else whoever is on the hook (`resource_assignments`), else every member holding `project:edit`, else the submitter with the waiver recorded as `selfReview` on the audit event. It never resolves to nobody, which is what used to make this a `422` on every path that had no way to name a reviewer. Opens an actionable `document.review_requested` inbox item per reviewer, anchored on the VERSION. | `{ reviewerUserIds?: string[] }` | `{ version }` |
| `POST` | `/api/documents/{id}/versions/{versionId}/approve` | Required | Freigeben. **A person's decision** (`actor: 'human'`), and the submitter cannot approve their own version — an assertion nobody but the author has read is not one. Two exceptions, both cases where that sentence is not what the row says: a version a RUN submitted (`submitted_by_actor = 'agent'`, migration 0085) names the commissioning human in `submitted_by` and not its author, so their Freigabe is the first human reading; and a one-person project has nobody else, which is waived with `selfReview` on the trail. Resolves the review round for every reviewer who was asked, not only the one who acted. | `{ comment? }` | `{ version }` |
| `POST` | `/api/documents/{id}/versions/{versionId}/changes` | Required | Änderungen anfordern. The comment is required by the schema AND by a CHECK on the row: it is quoted to the next turn and to the revision task. `delegateRevision` is the reviewer's THIRD action („Piloti überarbeiten lassen“) and not a third route — the version makes the same move either way, and the flag only changes the `openRevisionTask` effect's condition. Without it a version filed from a live conversation reaches that conversation as a `REVIEW_DECISIONS v1` block and opens no task; a version with no origin conversation opens one either way. | `{ comment, delegateRevision? }` | `{ version }` |
| `POST` | `/api/documents/{id}/versions/{versionId}/reject` | Required | Ablehnen. Reason required, for the same reason. A separate verb from `changes` because the two say different things to whoever wrote it: come back with a revision, or do not. | `{ comment }` | `{ version }` |
| `POST` | `/api/documents/{id}/versions/{versionId}/publish` | Required | Veröffentlichen. `project:documents:write`. Only an `approved` version can be published, and only a version a PERSON approved can be approved (`document_versions_published_is_approved`). One transaction supersedes the previous published version, swaps this one in and moves `documents.published_version_id` plus the item's storage columns; the superseded version keeps its bytes. **This is what puts a Piloti document in the index** (ADR-0054 § Indexing): the `ingestPublished` effect purges the superseded version's chunks and then dispatches the published version's bytes to `POST /v1/ingest`, under the row's `piloti/<document id>/<name>` filename and with the four provenance keys (`authored_by`, `approved_by` as the approver's display name, `approved_at`, `producer`). Nothing is dispatched for a document a PERSON wrote — its bytes were dispatched by whatever stored them. Best-effort in one direction only: the version row and the editorial act are durable whatever the index did with the bytes. | — | `{ version }` |
| `GET` | `/api/documents/{id}/versions/diff?from=&to=` | Required | Both versions' contents. The server deliberately does not diff: granularity, whitespace and move-detection are rendering decisions, and baking one in would freeze it for every later surface while buying nothing. | — | `{ from: { version, content }, to: { version, content } }` |
| `POST` | `/api/documents/{id}/archive` | Required | Take the item out of the working set. `project:documents:write`. Item-level, not a version state — „archiviert" is a statement about the FILE. The bytes and every version stay; this is not a delete. What DOES go is the chunks: a file that keeps coming back as a retrieval hit has not left the working set, so the passages are purged (`purgeIngestedChunks`, best-effort, recorded as `chunksPurged` on the audit event) for every document and not only a Piloti one. | `{}` | `{ documentId, lifecycle }` |

| `GET` | `/api/documents/{id}/versions/reviewers` | Required | Who this document's next version may be sent to: every member holding **`project:edit`**, minus the reader. The reviewer picker's list, and the list `POST /api/internal/document-versions` resolves the agent's `reviewer` NAME against. `project:edit` and not `project:view` on purpose — being able to open a project does not make somebody able to release a Brandschutzkonzept, and a picker built on the assignment candidates would offer people whose Freigabe the approve transition then refuses. | — | `{ candidates: { userId, name, email }[] }` |

Source: `frontends/ui/src/app/api/documents/[id]/versions/**`, `frontends/ui/src/app/api/documents/[id]/archive/route.ts`, `frontends/ui/src/lib/documents/lifecycle.ts`, `frontends/ui/src/lib/documents/version-content.ts`, `frontends/ui/src/lib/documents/reviewers.ts`.
...
| Method | Path | Auth | Description | Request Body / Params | Response |
|--------|------|------|-------------|-----------------------|----------|
| `GET` | `/api/ris/document` | Session only, rate-limited `ris-document` (60/min, burst 6/5s — the only read here that makes a third-party network call, and reads are not defaulted) | A document from the Austrian RIS as TEXT, so a RIS citation opens in Piloti's own reader instead of a browser tab (#622). Proxies the backend's internal `GET /v1/ris/document`, which reads it through the SAME `RisClient` the agent's `ris_fetch_document` tool uses — same host allow-list (`www.ris.bka.gv.at`, `ris.bka.gv.at`, `data.bka.gv.at`, https only, **re-validated on every redirect hop** — the check used to run on the caller's string alone, so a RIS endpoint reflecting a parameter into `Location` turned it into a formality), the same size ceiling, and the same shared `fetch_document_cached` read-through, so a document the answer was grounded on is usually served without touching RIS. An iframe was never an option: RIS sends `Content-Security-Policy: frame-ancestors 'none'` on every page. A reference the client refuses (non-RIS host, a document RIS publishes only as PDF/RTF, an unparseable document number) is a `404 NOT_FOUND` — a statement about the reference, not an outage; an unreachable RIS is `502 UPSTREAM_ERROR`. The reader keeps the outbound link in its header either way: RIS is the publication of record and this is a reading copy. Text is bounded at 1 000 000 characters; beyond that the window is centred on the cited `passage` when it can be located, because a document clipped at its HEAD drops the paragraph the citation points at (measured 2026-09-04: Bauordnung für Wien 759 595 characters with § 108 at 524 079; NÖ Bauordnung 2014 433 820; ABGB 971 067; ASVG 4 294 779). The bound is set by the CLIENT and not by the corpus: Chromium lays out a 4.3M-character `pre-wrap` node in 903 ms, but `locatePassageInText` folds every character before it, on the main thread, and that is the cost that grows — 106 ms at 600k and 386 ms at 2M on a server CPU. Centring the window on the passage is what makes tuning it from that side safe. `authz: { sessionOnly }`: RIS is public law and the bytes are the bytes at the URL the citation already shows, so there is no tenant resource to authorize against — only an origin that must not become a free fetcher for the internet, and the allow-list is enforced on the far side of the internal token. | `?reference=<RIS URL or document number>&application=` | `{ url, title, text, truncated }` |

Source: `frontends/ui/src/app/api/ris/document/route.ts` (client: `frontends/ui/src/lib/ris/document.ts`; backend: `frontends/aiq_api/src/aiq_api/routes/ris.py`). Rendered by `RisDocumentDialog` (`frontends/ui/src/features/knowledge/components/ris-document-dialog.tsx`), reached from any citation chip or inline `[N]` whose source resolves to a `ris` target.
...
| `POST` | `/api/projects/{id}/jobs` | project:skills:manage | Create. `prompt` is required (1–8000 chars); `skillName` is optional — when given it is resolved (org row first, builtin fallback; unknown → 404) and snapshotted, and `skill_name`/`skill_snapshot` are always written as a pair. **No `output`** — the service writes `deep-research` for every job it creates, so the body cannot ask for a run that files nothing. Validates cron (+ min interval, IANA timezone) and computes `next_run_at`. | `{ name, prompt, skillName?, dataSources?, enabled?, scheduleCron?, scheduleTimezone? }` | `{ job }` (201) |
...
|---|---|---|---|
| `GET` | `/api/projects/[id]/tasks` | Session, `project:view` | A project's delegated work, newest first (bounded): one row per run with the pinned requester (id AND resolved display name), lifecycle status, where the result went (`filedDocumentId`, `conversationId`) and the review. Gated on the skills feature like the jobs routes. **Projected explicitly (2026-09-10)** through `lib/tasks/list-projection.ts`: dates as ISO strings, and `plan.prompt` — which carries a skill's whole body — and the operator-facing `filingDetail` never cross. Read by the Aufgaben tab of a project's Automation section, which is the surface ADR-0051 deferred and this route had none of until now. |
...
### Research plans (ADR-0068)

The plan a deep research is about, and the run waits on. One contract
(`frontends/ui/src/lib/plans/plan-types.ts`, exported as
`tests/fixtures/research-plan.schema.json`), one typed browser client
(`lib/plans/plan-client.ts`).
...
creates the plan and commissions its run in one step.
...
| `POST` | `/api/internal/skills/fire` | `x-grid-internal-token` | Scheduler-fired run (`{ scheduleId }` — the pre-jobs spelling of a `task_definitions.id` since migration 0086, kept because the scheduler container and the BFF deploy separately). Re-checks `enabled` + the org's skills gate, then submits through the shared fire path (ADR-0046). |
| `POST` | `/api/internal/jobs/[jobId]/outcome` | `x-grid-internal-token` | The worker reports how a background run ended (`{ organizationId, status: success \| failure \| interrupted, error?, report?, cards? }`), by the backend job id — the one id it holds. ONE lookup into `task_runs` under narrow platform access (the two-branch `job_runs`-then-`tasks` fallback died with migration 0086), cross-checks the tenant, closes the run with `recordRunOutcome` (ADR-0051) — filing a finished deep-research report into the project as the run's pinned requester — then emits a `job.completed` / `job.failed` inbox item to that requester as the tenant, naming the filed document. 404 for a backend id with no run (an interactive deep-research job), which the worker treats as "nobody to tell". |
...
| `POST` | `/api/internal/document-versions` | `x-grid-internal-token` **plus a verified signed envelope** | **New 2026-09-10 (ADR-0054 §4).** The ONE route a machine may write a document version through. Op set is closed to `create \| update \| submit`; approve, request changes, reject, publish and archive have no internal route at all, and `route.spec.ts` asserts every op this admits maps to a transition row whose `actor` is `either`. The acting identity does NOT come from the body: the route verifies the `X-Grid-Request-Context` envelope the BFF minted at the start of the turn (`verifyGridRequestContextEnvelope`, HMAC-SHA256 over the raw JSON, keyed on `GRID_INTERNAL_API_TOKEN`, with a six-hour window whose `issuedAt` is INSIDE the signed bytes so a captured envelope cannot be refreshed) and builds a real, role-bearing session for the user it names with `resolvePinnedRequesterSession` (ADR-0051). Every permission check, `created_by` and audit actor downstream is that person; a requester who has left the organization is a 403. The tenant scope is opened from the verified envelope, never from the body. **`body.projectId` must equal the envelope's `projectId` when the envelope names one** (`requireEnvelopeProject`, 400 otherwise): every other field of the acting context comes from the signature, and `projectId` was the one a captured envelope could still be pointed at a second project the requester happens to be able to write to. An envelope with no project constrains nothing, and the body's own project is then gated by `requireProjectAccess` in the service exactly as before. No envelope, a tampered signature, one with no `issuedAt`, or one naming no user is a 401 that never says which. |
| `GET` | `/api/internal/document-versions/[versionId]/content?organizationId=&conversationId=` | `x-grid-internal-token` | **New 2026-09-10.** One version's text, plus the four facts a working-directory filing record is made of (`documentId`, `versionId`, `state`, `contentHash`) and the document's `filename` / `displayName`. Read by the turn's subject-document loader (`src/aiq_agent/turn/subject_document.py`) when the conversation's subject is a version nobody has published: only a published version reaches the retrieval index (ADR-0054), so a draft has no chunks and the focus filter falls open to the whole corpus — the turn reads the bytes into the conversation's working directory instead. Read-only, so NO signed envelope: it acts as nobody, unlike the `POST` above. **Both query parameters are required, and together they are the whole predicate.** `organizationId` is the tenant slot; `conversationId` names the conversation the turn is running in, and the route refuses unless that conversation's `subject_resource_type` is `document` and its `subject_resource_id` IS this version's document. The organization alone was not enough and could not be: a version id is a uuid the caller supplies rather than an unguessable collection name the way `/api/internal/document-file` is, so anything holding the internal token could name any tenant and read any version's bytes. The subject was written by a person's own session through the file-native ask, which makes the pair exactly the authority the turn already had. A version from another tenant, a version this conversation is not about, an ordinary chat, and an id that never existed all answer the same 404. |
| `POST` | `/api/internal/tasks` | `x-grid-internal-token` **plus a verified signed envelope** | **Op `plan` added 2026-09-22 (ADR-0068):** `{ op: "plan", projectId, plan, start: { policy: auto \| ask, graceSeconds? }, context? }` creates the research plan and commissions the run that waits on it; answers the `research` ids plus `planId`, `planStatus`, `startsAt`. **New 2026-09-10 (ADR-0051, ADR-0055).** The ONE route a machine may delegate work through — the `create_task` tool's only call. Op set is closed to the two asking verbs, `create` and `research`: a machine may ASK for work and may never judge it, so reviewing a task stays the session route `POST /api/projects/[id]/tasks/[taskId]/review`. Body is `{ op: 'create', projectId, kind, goal, due?, cadence?, cadenceTimezone? }` with `kind` one of `compliance_check | einreichcheck | document | revision` (`DELEGATABLE_TASK_KINDS`); `due` is `YYYY-MM-DD` (read as the END of that day) or a full ISO instant, and an unparseable one is dropped rather than refused; `cadence` is a 5-field cron (`0 8 * * 1` = Mondays 08:00) with `cadenceTimezone` its IANA zone (UTC when absent), and a definition is either one-off (`due`) or recurring (`cadence`), never both. Identity is the document-versions route's, through the same helper (`lib/api/internal-envelope.ts`): the verified `X-Grid-Request-Context` envelope names the acting person, `resolvePinnedRequesterSession` builds their real session, and the task's `requester_user_id` is pinned to them — so the run spends their budget and acts under their permissions. `project:edit` + `project:documents:write` on the named project, checked in that session — a cadence additionally requires `project:skills:manage`, because recurrence, not the asking, is the permissioned act. The tenant slot comes from the verified envelope, never from the body, and `body.projectId` must equal the envelope's `projectId` when it names one (`requireEnvelopeProject`, 400 otherwise) — a task queues a run that files as this person, so a replayed envelope pointed at a second project would be work nobody asked for, attributed to somebody who did not ask for it. Answers `{ taskId, kind, title, status, conversationId, dueAt, scheduled, nextRunAt }` with 201; `scheduled` is `true` for a cadence, meaning no run was dispatched — the scheduler owns every fire and `nextRunAt` names the first. **`op: 'research'`** is the escalation (ADR-0062): body `{ op: 'research', projectId, question }`, one `task_runs` row with NO definition behind it (a question asked in passing is not a standing intent), kind `deep-research`, dispatched as a `deep-research` output into the thread the envelope names. Same gate as `create` (`project:edit` + `project:documents:write`, `COMMISSION_PERMISSIONS`), same envelope rules; 409 when the envelope carries no `conversationId`, because a run is one message in the thread that commissioned it and there would be nowhere to narrate. Answers `{ runId, runMessageId, conversationId, status }` with 201 — `status` is `running`, or `failed` when the worker refused, which is a recorded run and not an error. |
| `POST` | `/api/internal/runs/by-job/[backendJobId]/report` | `x-grid-internal-token` | **New (ADR-0062).** A finished run's answer, written INTO the run's own message — the one assistant message the run has held in its thread since it was submitted, next to the ledger it has been growing. Body is `{ content, metadata? }`, `metadata` in the BACKEND's wire spelling (`sources`, `answer_confidence`, `research_truncated`), translated by `normalizeAgentAnswerMetadata` at this one door and merged per top-level key so the ledger is untouched. Keyed on the BACKEND job id because that is the only id a worker holds: the message id is derived from the `task_runs` id and only the BFF knows the pair. Identity is the ledger route's — organization, project and requester read off the `task_runs` row under narrow platform access, then the write done inside that run's own organization, and the body names nobody. **404 is an answer, not a failure**: a run with no message (an interactive deep-research job, or one submitted before this shipped) is what `conversation_output.py` reads as „write the turn the old way“, which is what lets the two services deploy in either order without losing a report. |
