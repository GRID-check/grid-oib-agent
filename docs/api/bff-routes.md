...
| `POST` | `/api/internal/skills/fire` | `x-grid-internal-token` | Scheduler-fired skill run (`{ scheduleId }`). Re-checks `enabled` + the org's skills gate, then submits through the shared fire path (ADR-0023 successor, Agent Skills). |
