...
| Method | Path | Auth | Description | Request Body / Params | Response |
|--------|------|------|-------------|-----------------------|----------|
| `GET` | `/api/ris/document` | Session only | A document from the Austrian RIS as TEXT, so a RIS citation opens in Piloti's own reader instead of a browser tab (#622). Proxies the backend's internal `GET /v1/ris/document`, which reads it through the SAME `RisClient` the agent's `ris_fetch_document` tool uses — same host allow-list (`www.ris.bka.gv.at`, `ris.bka.gv.at`, `data.bka.gv.at`, https only), same size ceiling, same shared cache, so a document the answer was grounded on is usually served without touching RIS. A reference the client refuses (non-RIS host, a document RIS publishes only as PDF/RTF, an unparseable document number) is a `404 NOT_FOUND` — a statement about the reference, not an outage; an unreachable RIS is `502 UPSTREAM_ERROR`. The reader keeps the outbound link in its header either way: RIS is the publication of record and this is a reading copy. Text is clipped at 400 000 characters with `truncated: true` rather than ending mid-sentence. `authz: { sessionOnly }`: RIS is public law and the bytes are the bytes at the URL the citation already shows, so there is no tenant resource to authorize against — only an origin that must not become a free fetcher for the internet, and the allow-list is enforced on the far side of the internal token. | `?reference=<RIS URL or document number>&application=` | `{ url, title, text, truncated }` |
