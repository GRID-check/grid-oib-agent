...
A reader shown a plain success who then finds nothing in Berichte has no way to
learn why — the only record was a server log. Filing itself stays best-effort:
none of this fails the request, and the user waited minutes for the report and
gets it regardless.
