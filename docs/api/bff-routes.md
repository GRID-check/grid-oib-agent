...
| `POST` | `/api/documents/{id}/versions/{versionId}/changes` | Required | Änderungen anfordern. The comment is required by the schema AND by a CHECK on the row: it is quoted to the next turn and to the revision task. | `{ comment }` | `{ version }` |
