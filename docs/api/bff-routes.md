...
| `POST` | `/api/internal/jobs/[jobId]/outcome` | `x-grid-internal-token` | The worker reports how a background run ended (`{ organizationId, status: success \| failure \| interrupted, error? }`), by the backend job id — the one id it holds. Looks the `job_runs` row up under narrow platform access, cross-checks the tenant, then emits a `job.completed` / `job.failed` inbox item to the job's creator as that tenant. 404 for a backend id with no run (an interactive deep-research job), which the worker treats as "nobody to tell". |
