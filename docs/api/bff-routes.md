...
| `POST` | `/api/documents/{id}/versions/{versionId}/submit` | Required | Hand the version to a reviewer. `project:edit` / `project:documents:write`. `reviewerUserIds` may be empty, in which case the document's assignees (`resource_assignments`) are asked; both empty is a `422`, because a submission that reaches no inbox looks exactly like one that worked. Opens an actionable `document.review_requested` inbox item per reviewer, anchored on the VERSION. | `{ reviewerUserIds?: string[] }` | `{ version }` |
| `POST` | `/api/documents/{id}/versions/{versionId}/approve` | Required | Freigeben. **A person's decision** (`actor: 'human'`), and the submitter cannot approve their own version — an assertion nobody but the author has read is not one. Resolves the review round for every reviewer who was asked, not only the one who acted. | `{ comment? }` | `{ version }` |
...
Source: `frontends/ui/src/app/api/documents/[id]/versions/**`, `frontends/ui/src/app/api/documents/[id]/archive/route.ts`, `frontends/ui/src/lib/documents/lifecycle.ts`.
