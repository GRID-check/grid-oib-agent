...
| `POST` | `/v1/jobs/async/job/{job_id}/cancel` | Cancel a running job. Idempotent (#632): cancelling an already-terminal job is a no-op `200` returning its status (`task_cancelled: false`), not a `400` — the UI races completion with user cancel, session-delete and purge cancels. | — | `{ job_id, status, task_cancelled }` | Same |
