...
| Method | Path | Description | Request | Response | Handler |
|--------|------|-------------|---------|----------|---------|
| `DELETE` | `/v1/drafts/{conversation_id}` | Drops one conversation's working directory (the LangGraph store namespace the chat agent writes drafts into). Internal token only; `204`; idempotent. Called last by the BFF's conversation deletion, best-effort. | — | — | `aiq_api.routes.drafts` |
