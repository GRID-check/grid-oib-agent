...
| Method | Path | Description | Request | Response | Handler |
|--------|------|-------------|---------|----------|---------|
| `POST` | `/v1/collections/{collection_name}/search` | Deterministic semantic vector search over one collection's already-embedded chunks (**no LLM, no agent loop**). Retrieves the top `top_k` chunks via the cached retriever singleton, then aggregates document-centric: one hit per `file_name` (its max-score chunk → `score`, `snippet` ≈300 chars, `page_number`), sorted by score descending, capped at `top_k_files`. `404` if the collection is missing; `422` on empty query. | `{ query (1–1000), top_k=40 (1–100), top_k_files=20 (1–100) }` | `{ hits: [{ file_name, score, snippet, page_number, collection }] }` | `add_document_search_routes` in `aiq_api.routes.document_search` |
