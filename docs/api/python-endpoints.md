...
| Method | Path | Description | Request | Response | Handler |
|--------|------|-------------|---------|----------|---------|
| `POST` | `/v1/ingest` | Ingest a file from a presigned URL | `{ file_ref, collection, document_id?, thumbnail_upload_url?, folder_path?, file_name?, authored_by?, approved_by?, approved_at?, producer? }` | `{ job_id, status, document_id }` (202) | `add_ingest_routes` in `aiq_api.routes.ingest` (`frontends/aiq_api/src/aiq_api/routes/ingest.py`) |

Downloads the file from `file_ref` (presigned SeaweedFS URL), saves to a temporary file, infers extension from `Content-Type` or URL path, then submits to the ingestor's `submit_job()`. The BFF upload route (`POST /api/documents/upload`) calls this endpoint after writing to SeaweedFS. `document_id` rides into the job config: the LlamaIndex pipeline uses it to ask the BFF for a presigned PUT per embedded raster it extracts (`POST /api/internal/document-image-upload-url`), storing the image beside the document for `view_knowledge_image`; a caller that sends none (the OIB corpus sync) gets captions only.
...
`file_name` is the document's identity inside the collection — the BFF's `documents.filename`, which is the join key every chunk purge addresses. Omit it and the name is derived from the presigned URL's last path segment, which is the OBJECT KEY's basename: the two differ whenever the BFF sanitised the name to build the key (a slash becomes an underscore, a 300-character name is cut) and for a namespaced Piloti document, so chunks filed under the derived form are chunks nothing can purge. The one caller that deliberately omits it is the IFC digest dispatch, which genuinely ingests a different file than the row names.

`authored_by`, `approved_by`, `approved_at` and `producer` are the provenance of a document Piloti wrote and a person released (**ADR-0054**). `authored_by: "agent"` is the only value acted on; anything else, absence included, is a human document and the other three are dropped with it. They are carried into the detached ingest thread and stamped onto **every chunk's metadata** and onto the `document_metadata` row's `provenance` column, read back by `aiq_agent.common.provenance`. See [`../architecture/agent-document-provenance.md`](../architecture/agent-document-provenance.md).
...
| Method | Path | Description | Request | Response | Handler |
|--------|------|-------------|---------|----------|---------|
| `DELETE` | `/v1/drafts/{conversation_id}` | Drops one conversation's working directory (the LangGraph store namespace the chat agent writes drafts into). Internal token only; `204`; idempotent — already-gone answers `204` like just-deleted. Called last by the BFF's conversation deletion, best-effort. | — | `204` empty | `add_draft_routes` in `aiq_api.routes.drafts` (`frontends/aiq_api/src/aiq_api/routes/drafts.py`) |
| `GET` | `/v1/drafts/{conversation_id}` | Lists one conversation's drafts as `{path, bytes, version}` without content — or reads one draft via `?path=`. Internal token only. | `?path=` optional, e.g. `/entwuerfe/befund.md` (bare `entwuerfe/…` gains its leading slash) | List `{ conversation_id, drafts: [{ path, bytes, version }] }` sorted by path; single `{ path, content, version, filing? }` | Same |
| `GET` | `/v1/drafts/{conversation_id}/{file_path:path}` | Reads one conversation's draft by path suffix. Same single shape and same guards as `?path=`. Internal token only. | Path suffix, e.g. `/v1/drafts/{id}/entwuerfe/befund.md` | `{ path, content, version, filing? }` (`filing` present only when the draft has been filed) | Same |
...
**Draft auth**: all three doors call `_require_internal_token` (`aiq_api.routes.internal_auth`) — internal token matching `GRID_INTERNAL_API_TOKEN`, sent as **`X-Grid-Internal-Token`** (`X-Internal-Token` also accepted); dev-default refused outside dev; never on the AuthMiddleware external-path allowlist. Missing/mismatched token → `403`; dev-default token outside a dev `APP_ENV` → `503`.

**Draft reads are read-only**: the `GET` doors call `asearch`/`aget` and nothing else, so they cannot write the store, and they never touch `grid_app` (single-writer rule, ADR-0003). The list carries no content — `bytes` is the draft's byte size, `version` the stored version counter (`0` when absent) — and entries are sorted by path. A single read returns the stored text as `content` plus `version`, and the `filing` record when the draft has been filed.

**Draft validation and errors**: the `GET` doors reject a conversation id that is not one namespace element — blank/whitespace-only, over 200 chars, `.`/`..`, `..` anywhere, or containing `/` or `\` — with `400` (`Invalid conversation id`); a path outside the working directory or containing `..` is refused with `400` (`Invalid draft path`) by the SAME `draft_store.path_refusal` the write verbs enforce, so a read can never name a file a write could not have created. (`DELETE` skips the id-shape check: it sweeps the namespace and answers `204` either way.) A well-formed address with nothing under it is `404` — empty list → `404` (`No drafts for this conversation`), unknown single path → `404` (`Draft not found`) — the same nothing-to-discard convention the `DELETE` callers already treat as success. A single read past the drafts ceiling (256 KB, `MAX_DRAFT_BYTES`) answers `413` rather than streaming unbounded bytes. A store failure is `500` (`Failed to read drafts` / `Failed to delete drafts`), logged with the conversation id.
...
| `POST` | `/v1/jobs/async/job/{job_id}/cancel` | Cancel a running job. Idempotent (#632): cancelling an already-terminal job is a no-op `200` returning its status (`task_cancelled: false`), not a `400` — the UI races completion with user cancel, session-delete and purge cancels. `already_terminal` is `true` on that no-op and is the field to branch on: `task_cancelled` is `false` for a cancel that took under DB-claimed execution, and `status` is `interrupted` both for a fresh cancel and for a second one. The BFF's run cancel (`cancelRun`) maps it back to its `409`. | — | `{ job_id, status, task_cancelled, already_terminal }` | Same |
...
- **Agent types**: Registered via `register_agent()` in `aiq_api.registry`. Default agents: `deep_researcher`, `researcher`.
...
|--------|------|------|-------------|---------|----------|---------|
| `POST` | `/v1/internal/skills/submit` | Internal token matching `GRID_INTERNAL_API_TOKEN`, sent as **`X-Grid-Internal-Token`** (`X-Internal-Token` also accepted — see the note below); dev-default refused outside dev; not on the external-path allowlist | Submit a **job** run as an async job on behalf of the job's owner (no user JWT — called by the BFF fire path for scheduled and manual runs). Wraps `submit_agent_job`, so admission control, cost tracking (explicit identity), and `job_access` ownership apply exactly like the public submit route. Agent selection is deterministic from the JOB's `output` (`chat` → `researcher`, `deep-research` → `deep_researcher` — `_OUTPUT_AGENT_TYPES`) unless `agent_type` overrides it; nothing is read off the skill. `skills` is the attached skill's name or an **empty list** (a job may attach none, and then the prompt runs alone); it is a name list for the log only, because the skill's BODY already rides inside the composed `input` and nothing is forced onto the worker's agent state. `conversation_id` is the conversation the BFF created for an `output='chat'` run to land in; absent for deep-research and for any run whose conversation could not be created. `run_id` is the `task_runs` row the job IS (ADR-0062): the worker wraps its event store in a ledger fold and flushes the run's account onto that run's own message; without it the job still narrates itself on its event stream and writes no ledger. | `{ input, skills, output, agent_type?, job_id?, data_sources?, collection_scope?, project_context?, organization_id, user_id?, project_id?, conversation_id?, run_id?, owner_email?, budget_header?, model_overrides? }` — `input` is the composed prompt (job prompt + the attached skill's body), max 48000 chars | `{ job_id }`; 429 + `Retry-After` (caps), 409 (duplicate job id), 503 (Dask/scheduler unconfigured), 422 (invalid payload / unknown data sources / neither `output` nor `execution`), 400 (unknown agent), 403 (token) | `add_skill_routes` in `aiq_api.routes.skills` |
...
|---|---|---|---|---|---|---|
| `GET` | `/v1/ris/document` | Internal token matching `GRID_INTERNAL_API_TOKEN`, sent as `X-Grid-Internal-Token` (`X-Internal-Token` also accepted); dev-default refused outside dev; **not** on the external-path allowlist | A RIS document as plain TEXT, so a RIS citation opens in Piloti's own reader instead of a browser tab (#622 — RIS sends `Content-Security-Policy: frame-ancestors 'none'` on every page, so an iframe is not an option and never was). Reads through the SAME `RisClient` and the same `fetch_document_cached` read-through the agent's `ris_fetch_document` tool uses: one host allow-list (`www.ris.bka.gv.at`, `ris.bka.gv.at`, `data.bka.gv.at`, https only, **re-validated on every redirect hop**), one size ceiling, one shared Dragonfly cache — so a document the answer was grounded on is usually served without touching RIS, and there is no second extractor to keep in step. The client is held for the process, not built per request. Text is bounded at 1,000,000 characters; over that the window is centred on `passage` when it can be located, because a document clipped at its head drops the very paragraph the citation points at (measured: the Bauordnung für Wien is 759,595 characters and its § 108 sits at 524,079; the ABGB 971,067; the ASVG 4,294,779). The bound is a CLIENT budget — the reader's passage matcher folds every character on the main thread — not a corpus one; centring on the passage is what makes it safe to set from that side. | `?reference=<RIS URL or document number>&application=&passage=` | `{ url, title, text, truncated }`; 400 (a reference the client refuses — non-RIS host, PDF-only document, unparseable number), 502 (RIS unreachable), 403 (token) | `add_ris_routes` in `aiq_api.routes.ris` |
