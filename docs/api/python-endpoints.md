...
|--------|------|------|-------------|---------|----------|---------|
| `POST` | `/v1/internal/skills/submit` | Internal token (`X-Internal-Token` matching `GRID_INTERNAL_API_TOKEN`; dev-default refused outside dev; not on the external-path allowlist) | Submit a skill run as an async deep-research job on behalf of the skill's owner (no user JWT — called by the BFF fire path for scheduled and manual skill runs). Wraps `submit_agent_job`, so admission control, cost tracking (explicit identity), and `job_access` ownership apply exactly like the public submit route. Agent selection is deterministic from the skill's declared `grid-execution` mode (`chat` → `shallow_researcher`, `deep-research` → `deep_researcher`) unless `agent_type` overrides it; the submitted job carries the skill names in `force_skills` so the worker force-activates them. | `{ input, skills, execution, agent_type?, job_id?, data_sources?, collection_scope?, project_context?, organization_id, user_id?, project_id?, owner_email?, budget_header?, model_overrides? }` | `{ job_id }`; 429 + `Retry-After` (caps), 409 (duplicate job id), 503 (Dask/scheduler unconfigured), 422 (invalid payload / unknown data sources), 400 (unknown agent), 403 (token) | `add_skill_routes` in `aiq_api.routes.skills` |
