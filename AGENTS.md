...
| Change what a customer can **see** | Capture it and attach it to the PR — `agent-browser` to shoot, `before-and-after` to publish. Commit no image files. [`docs/ux/visual-screenshots.md`](docs/ux/visual-screenshots.md) | The **Require a capture for user-visible changes** step in CI. Escape hatch: the `no-visual-evidence` label, or `<!-- no-visual-evidence: why -->` in the PR body (the reason is required) |
