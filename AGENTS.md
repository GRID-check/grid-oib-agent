...
| `GRID_ORG_ARCHIV_ENABLED` | Default `false`. Non-enforced-flags fallback for the org-wide document **Archiv** (ADR-0024): shows the `/app/archiv` page + nav entry, enables the `/api/archiv/*` routes, and injects the org's `archiv_<orgId>` collection into every project's retrieval scope. With `GRID_ENFORCE_FEATURE_FLAGS=true`, the per-org `organization-archiv` WorkOS flag controls it instead. Reads are open to any org member; uploads/deletes require the `org:archiv:manage` permission. Frontend service. |
...
| `GRID_NORMS_DIR` | Default `configs/norms`. Root of the per-country norm registry (ADR-0025, `configs/norms/<country>/registry.yml`) — the typed spine for all known legal documents (rank/role/editions/relations/applicability), consumed by the `ris_search` short-circuit, the `ris_catalog_lookup` tool (incl. relations), and the lane-rendered researcher prompt block. Missing/invalid = registry features disabled, live RIS search unaffected (fail-open). RIS pointers re-verified + merged with `scripts/build_ris_catalog.py`. Backend (aiq-agent) service. |
| `GRID_RIS_CACHE_TTL_DAYS` | Default `7`. Days a fetched RIS full text in the persistent `ris_knowledge` cache (`data/ris_cache`, ADR-0025 Phase 3) is served without re-validation. Past the TTL, `ris_fetch_document` cheaply re-validates the Fassung date and only re-fetches on an actual Novelle; `0`/invalid falls back to `7`. Backend (aiq-agent) service. |
| `RIS_CATALOG_PATH` | **Deprecated (ADR-0025).** The flat `configs/ris_catalog.yml` is retired; when set, the `ris_catalog` shim loads that legacy file with a `DeprecationWarning`, otherwise the catalog view derives from the norm registry. Migrate overrides to `configs/norms/<country>/registry.yml`. Backend (aiq-agent) service. |
