...
| `CONSISTENCY_LLM_MODEL` / `CONSISTENCY_LLM_API_KEY` / `CONSISTENCY_LLM_BASE_URL` | LLM used by the end-of-wizard free-text intake consistency-check endpoint (`POST /v1/consistency-check`). Each falls back to the generic `LLM_MODEL`/`LLM_API_KEY`/`LLM_BASE_URL`, then to the OpenRouter defaults (`deepseek/deepseek-v4-flash` @ `https://openrouter.ai/api/v1` using `OPENROUTER_API_KEY` when set, else `gpt-4o-mini` @ `https://api.openai.com/v1`). Best-effort: with no key resolvable the endpoint returns `error=llm_not_configured` (HTTP 200) and the wizard saves anyway. Backend (aiq-agent) service. |
| `BACKFILL_SUMMARY_API_KEY` / `BACKFILL_SUMMARY_BASE_URL` / `BACKFILL_SUMMARY_MODEL` | Credentials for the one-off `scripts/backfill_document_tags.py` tagging LLM (runs outside the NAT runtime, so it builds an OpenAI-compatible client from env). `BACKFILL_SUMMARY_API_KEY` is required (falls back to `NVIDIA_API_KEY`); missing → the script exits `2`. `BACKFILL_SUMMARY_BASE_URL` default `https://integrate.api.nvidia.com/v1`; `BACKFILL_SUMMARY_MODEL` default `nvidia/nemotron-mini-4b-instruct`. Must match the config's `summary_llm` block. |
