...
| `AIQ_VIEW_IMAGES_ENABLED` | Default `true`. Enables the `view_knowledge_image` NAT tool (ADR-0039): re-renders an ingested PDF page on demand (pypdfium2 → JPEG, long edge `AIQ_PAGE_RENDER_MAX_DIM`, default `2048`px) and returns it as a **multimodal image block during a research turn**, so the model sees plans/drawings at answer time, not only at ingestion. Effective only when a VLM key resolves (`AIQ_VLM_API_KEY`); every failure path degrades to a text-only explanation block. `false` disables the tool. Backend (aiq-agent) service. |
