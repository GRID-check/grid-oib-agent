...
| `GRID_IFC_MODELS_ENABLED` | Default `false`. Dark-launch gate for IFC/BIM models (ADR-0045: `.ifc` upload, the model workspace, the WebGPU viewer, the Prüfbuch, the agent's `ifc_query` tool). Puts `.ifc` in the upload accept-list, shows the Model nav entry + page, and enables every `/api/**/bim/*` route while `GRID_ENFORCE_FEATURE_FLAGS` is off; with enforcement on, the per-org `ifc-models` WorkOS flag controls them. Unlike an ordinary flag this is **default-deny rather than fail-open** — the feature renders OIB compliance verdicts, so an operator must choose it. No paired capability var: extraction runs in the BFF process and the viewer in the browser. Frontend service. |
...
| `BIM_MAX_IFC_BYTES` / `BIM_ELEMENT_LIMIT` | IFC extraction bounds (ADR-0045). `BIM_MAX_IFC_BYTES` (default 250 MB) is the largest `.ifc` the BFF will parse — parsing runs in the Node process, so a bigger model is refused with a readable reason instead of taking the worker down. `BIM_ELEMENT_LIMIT` (default 200 000) caps `bim_elements` rows per model; the summary's counts and totals stay EXACT above the cap and `summary.truncatedAt` records that the element LIST was cut. Frontend (BFF) service. |
