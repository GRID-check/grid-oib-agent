...
| `SEAWEED_PUBLIC_ENDPOINT` | Browser-reachable SeaweedFS endpoint for presigned preview/download URLs (dev default `http://localhost:8333`). Frontend service, **and since the code-scanning sweep also the aiq-agent tier**: the `/v1/ingest` SSRF guard (`aiq_api.routes.ingest._object_store_hosts`) trusts exactly the hosts of `SEAWEED_ENDPOINT` + `SEAWEED_PUBLIC_ENDPOINT`, and the UI presigns against the public host — so a backend that lacks this variable rejects every presigned ingest URL. |
| `SEAWEED_PER_ORG_BUCKETS` | Default `false`. `true` writes each organization's objects into its own bucket instead of a key prefix inside `SEAWEED_BUCKET` (ADR-0043): erasing a tenant becomes one `DeleteBucket` instead of a paginated sweep that can half-finish, and a key-construction bug stops being a cross-tenant bug. **Not a cutover** — the bucket is recorded per row (`documents.storage_bucket`, NULL = the shared bucket), so flipping it changes where the next object goes and leaves everything already written readable in place. **Frontend service only** — the purger needs neither this flag nor a naming rule: it reads the buckets a project's documents recorded (`SELECT DISTINCT storage_bucket`), which finds buckets written under a previous prefix and cannot disagree with the code that wrote them. On Kubernetes set via `grid-oib:seaweedfsPerOrgBuckets`. |
| `SEAWEED_TENANT_BUCKET_PREFIX` | Default `grid-org-`. Leading segment of a tenant bucket name; the rest is the slugged org id plus a 128-bit SHA-256 of the original id. The naming rule is one module — `src/lib/storage/bucket.ts` — and **the purger does not use it**: it reads the buckets its documents recorded instead, which is both more complete (it finds buckets written under a previous prefix) and impossible to get wrong by disagreeing with the writer. Also the string the tenant S3 grants are wildcarded on (`Read:<prefix>*`), so it must never be a prefix of a platform bucket name. **Frontend service only.** |
| `SEAWEED_TENANT_ADMIN_ACCESS_KEY` / `SEAWEED_TENANT_ADMIN_SECRET_KEY` | Credential for the only identity scoped to create and drop tenant buckets (`Admin:<prefix>*`). A separate key rather than a second client on the ordinary one, because SeaweedFS's `Admin:<bucket>` authorises CreateBucket and DeleteBucket together and cannot express one without the other — a distinct credential is the only way to keep "drop a tenant" off the request path. **Frontend service only**: the purger gets none of the three, since an unattended queue worker is the last process that should be able to drop a bucket. Note that the frontend DOES hold it — bucket creation is lazy, inside the upload request — so a frontend compromise can delete tenant buckets; ADR-0043 records that residual risk and what bounds it. Unset falls back to `SEAWEED_ACCESS_KEY`/`SEAWEED_SECRET_KEY`, which is what a deployment with per-org buckets off has (nothing calls it there). |
...
| `GRID_MAX_ACTIVE_JOBS` / `GRID_MAX_ACTIVE_JOBS_PER_ORG` | Defaults `8` / `3`. Admission control for async research jobs (global / per-org caps); beyond a cap, submits get 429 / a friendly chat message. `0` disables. Scheduled skill runs (ADR-0046) go through the same caps; rejected occurrences are recorded as `skipped` runs. |
...
| `GRID_ATTACHMENT_INDEX_WAIT_SECONDS` | Default `8`. Seconds a chat turn may wait for a file the user just attached to appear in `available_documents` (ADR-0048). The inventory is written by the async ingest job and the composer does not gate send, so on the first prompt after a drop the document the turn is *about* is routinely missing from it. Only a wire-declared **`indexing`** attachment that the inventory does not list is waited for, re-reading every 500 ms; `0` disables the wait, and an invalid/negative value falls back to `8` rather than disabling it. Fail-open and never raises — on timeout the turn proceeds with the inventory it has, and the researcher prompt already tells the agent to say the file is still being processed instead of answering from another document. **It runs inside the turn**, which holds a `GRID_MAX_ACTIVE_TURNS` admission lease for its duration; that is why it is bounded and disablable. Backend (aiq-agent) service. |
...
| `GRID_TRACE_IDENTITY_ATTRIBUTES` | Default unset = OFF. Kubernetes-only, Pulumi-injected, and keyed off the **Langfuse** tier (ADR-0044) rather than observability generally. When `true` the NAT tracing pipeline stamps `langfuse.user.id`, `langfuse.session.id` and the organization onto every span (`src/aiq_agent/observability/langfuse_trace_attributes.py`), which is what turns anonymous spans into traces you can attribute to a user and a tenant. Off by default because it is a privacy posture, not a performance knob: ADR-0029 accepted that traces carry user CONTENT on the grounds that access is gated to platform operators; making every span attributable to a named individual is a further step that should arrive with the product decision needing it. The processor runs AHEAD of NAT's redaction processor, so these attributes are redactable like any other (attributes added after redaction never could be). Session grouping and input/output need no flag — NAT already emits `session.id` and OpenInference `input.value`/`output.value`, which Langfuse maps natively. Backend (aiq-agent) + agent-worker services. |
...
| `GRID_IFC_MODELS_ENABLED` | Default **`true`** — IFC/BIM models (ADR-0045: `.ifc` upload, the model workspace, the WebGPU viewer, the Prüfbuch, the agent's `ifc_query` tool) ship ON. Governs `.ifc` in the upload accept-list, the Model nav entry + page and every `/api/**/bim/*` route while `GRID_ENFORCE_FEATURE_FLAGS` is off; with enforcement on, the per-org `ifc-models` WorkOS flag controls them. It has its own variable rather than riding the fail-open path so a deployment can set `false` to withdraw the feature without enabling flag enforcement globally. No paired capability var: extraction runs in the BFF process and the viewer in the browser. Frontend service. |
| `GRID_SKILLS_ENABLED` | Default `false`. Dark-launch gate for Agent Skills (ADR-0046: the org skill toolbox, `/name` invocation in chat, project skill schedules): shows the Skills section + every `/api/skills/*` and skill-schedule route while `GRID_ENFORCE_FEATURE_FLAGS` is off, and is the `skill-scheduler` worker's start gate — when neither this nor flag enforcement is on, the scheduler logs why and exits 0 instead of polling for work nobody can create. With enforcement on, the per-org `skills` WorkOS flag controls the UI/API instead. Frontend + skill-scheduler services. |
| `GRID_SKILL_SCHEDULER_POLL_MS` / `GRID_SKILL_SCHEDULER_BATCH` / `GRID_SKILL_RUNS_RETENTION_DAYS` | Defaults `30000` / `20` / `90`. Skill-scheduler knobs: tick interval, max due schedules claimed per tick (`FOR UPDATE SKIP LOCKED`), and run-history retention. skill-scheduler service. |
| `GRID_SKILLS_CACHE_TTL_SECONDS` | Default `60`. Seconds an organization's resolved skill set (builtin + org, shadowing applied) is cached in the shared Dragonfly/Redis cache (`aiq_agent.skills.resolver`, ADR-0020) before re-resolving via the BFF internal endpoint — cutting per-turn resolution across replicas and restarts. Cache-only/fail-open: a miss or fetch error falls back to the builtin set. `0`/invalid falls back to `60`. Backend (aiq-agent) service. See `docs/architecture/agent-skills.md`. |
| `GRID_SKILL_MIN_INTERVAL_MINUTES` | Default `15`. Minimum cron cadence accepted when saving a skill schedule (validated in the BFF). Frontend service. |
...
| `BIM_MAX_IFC_BYTES` / `BIM_ELEMENT_LIMIT` | IFC bounds (ADR-0045). `BIM_MAX_IFC_BYTES` (default 250 MB) is **three** limits in one number so they cannot drift apart: the upload ceiling for a `.ifc`, the largest file the BFF will parse, and the server's request-body ceiling via `next.config.ts` (`max(FILE_UPLOAD_MAX_SIZE_MB, BIM_MAX_IFC_BYTES)` — a transport limit below the validated one means the body is truncated before the handler and `formData()` throws). Deliberately separate from `FILE_UPLOAD_MAX_SIZE_MB` (100 MB, sized for documents) because a building model is routinely 50–500 MB. `BIM_ELEMENT_LIMIT` (default 200 000) caps `bim_elements` rows per model; the summary's counts and totals stay EXACT above the cap and `summary.truncatedAt` records that the element LIST was cut. Frontend (BFF) service. |
