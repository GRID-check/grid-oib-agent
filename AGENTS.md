...
| `RIS_CATALOG_PATH` | Default `configs/ris_catalog.yml`. Path of the curated RIS index (verified pointers to the building-relevant Austrian norms) consumed by the `ris_search` short-circuit, the `ris_catalog_lookup` tool, and the researcher prompt block. Missing/invalid file = feature disabled, live RIS search unaffected (fail-open). Regenerate with `scripts/build_ris_catalog.py`. Backend (aiq-agent) service. |
