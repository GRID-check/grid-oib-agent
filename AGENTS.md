...
| `GRID_SKILLS_ENABLED` | Default `false`. Dark-launch gate for the Agent Skills feature (scheduled skill runs, the ADR-0023 successor): while `GRID_ENFORCE_FEATURE_FLAGS` is off it is the `skill-scheduler` worker's start gate — when neither this nor flag enforcement is on, the scheduler exits cleanly at boot. skill-scheduler service. |
| `GRID_SKILL_SCHEDULER_POLL_MS` / `GRID_SKILL_SCHEDULER_BATCH` / `GRID_SKILL_RUNS_RETENTION_DAYS` | Defaults `30000` / `20` / `90`. Skill-scheduler knobs: tick interval, max due schedules claimed per tick (`FOR UPDATE SKIP LOCKED`), and run-history retention. The deprecated `GRID_WORKFLOW_SCHEDULER_*` / `GRID_WORKFLOW_RUNS_RETENTION_DAYS` names are still read as fallbacks for one release (with a warning). skill-scheduler service. |
