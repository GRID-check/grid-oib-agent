...
| `GRID_AGENT_AUTHORED_DOCUMENTS_ENABLED` | Default **`true`** — Piloti filing what it writes (a finished Recherchebericht, a drawn diagram) into a project ships ON. Gates the one filing seam (`lib/documents/generated.ts`), so every producer rides it, while `GRID_ENFORCE_FEATURE_FLAGS` is off; with enforcement on the per-org `agent-authored-documents` WorkOS flag decides. The OPERATOR's lever — the tenant's is the `project:documents:generate` permission, which is required in addition to `project:documents:write` and is withheld on a custom project role. Frontend service. |
