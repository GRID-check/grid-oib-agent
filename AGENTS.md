...
| `BIM_MAX_IFC_BYTES` / `BIM_ELEMENT_LIMIT` | IFC bounds (ADR-0045). `BIM_MAX_IFC_BYTES` (default 250 MB) is **both** the upload ceiling for a `.ifc` and the largest file the BFF will parse — one number so the two layers cannot disagree, and deliberately separate from `FILE_UPLOAD_MAX_SIZE_MB` (100 MB, sized for documents) because a building model is routinely 50–500 MB. `BIM_ELEMENT_LIMIT` (default 200 000) caps `bim_elements` rows per model; the summary's counts and totals stay EXACT above the cap and `summary.truncatedAt` records that the element LIST was cut. Frontend (BFF) service. |
