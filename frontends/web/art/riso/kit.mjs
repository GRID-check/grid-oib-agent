...
export function fileVersion(file) {
  try { return crypto.createHash('sha256').update(fs.readFileSync(file)).digest('hex').slice(0, 8); } catch { return null; }
}

/**
 * The manifest: one entry per site art id, keyed by id, files per density.
 * A pure function of the JOBS tables and the bytes in public/art/, so
 * check.mjs can rebuild it and compare.
 *
 ...
 */
export function manifestFrom(jobsByWork, version = (j) => fileVersion(destOf(j))) {
  const art = {}, missing = [];
  ...
  return {
    manifest: {
      $comment: 'Generated by art/riso/export.mjs from each work\'s JOBS table and the bytes in public/art/. Do not edit: change the work, then run node art/riso/export.mjs.',
      art: sorted,
    },
    missing,
  };
...
/** A manifest src without its ?v= query: the path under public/. */
export const srcPath = (src) => src.replace(/\?.*$/, '');
