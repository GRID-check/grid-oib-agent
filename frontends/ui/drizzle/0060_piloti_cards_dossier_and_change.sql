-- Three new card types get their craft, and the Bedingungsbaum gets its Vorfrage.
--
-- `document_checklist`, `deadline_timeline` and `change_impact` landed with a
-- trigger line each in `render_card_doctrine()`, which is all the always-on tool
-- description may carry — a ceiling test fails if craft drifts back into it. The
-- paragraphs saying when each actually earns its place belong here, in the skill
-- applied on every answering turn that a platform owner can edit without a
-- deploy. This is the rule 3f8c8e4a established and the one a new card type is
-- most likely to half-follow, because the trigger line is what gets the card
-- emitted at all and the paragraph costs nothing visible to omit.
--
-- What each paragraph is FOR, since none is obvious from the shape:
--
-- `document_checklist` — the answer to „welche Unterlagen brauche ich" is a list
-- of STATES, not of names: always-required vs conditional (and on what), who
-- issues it, and what the reader already holds. The card totals those states, so
-- a guessed `status` corrupts the tally above it and not merely its own row.
--
-- `deadline_timeline` — one Frist is a Hinweis; several in sequence make the
-- ORDER and the event each clock starts from the actual answer. It repeats the
-- „never a computed date" rule the Verfahrensablauf section already states,
-- deliberately: the two cards are reached from different questions and this is
-- the one error either can make that the prose beside it cannot correct.
--
-- `change_impact` — „was passiert, wenn X sich ändert" is planned against, so
-- every consequence carries its own Fundstelle or is left out, and „unverändert"
-- is half the answer rather than a wasted row. Kept distinct from the
-- Bedingungsbaum, which says which case the project is IN.
--
-- The fourth paragraph is a DISCRIMINATOR rather than a card description, so it
-- sits inside the existing selection section as a `###` subsection instead of
-- opening a fourth. It answers the question that has to be asked BEFORE a
-- Bedingungsbaum is built — can more than one row apply at once — and names the
-- observed defect: three simultaneously marked branches read as a decision that
-- was never taken. GK 4 Feuerwiderstand is the worked case, and it is a
-- typisierte Tabelle.
--
-- One clause was tempered on the way in. `document_checklist` opened by calling
-- its question „eine der häufigsten überhaupt", which is a frequency argument for
-- emitting — and the Verfahrensablauf section two headings above already opens
-- with that exact move. The product owner's reading of the fleet is that cards
-- "do get put out quite good", so the general rate is not what needs pushing;
-- the discrimination between names and states is the paragraph's real content
-- and it survives intact. Nothing else was softened: measured over the four new
-- blocks, restraint and honesty outweigh invitation 4.6 : 1, which moves the
-- whole body from 1.50 : 1 to 1.72 : 1 — back toward the 2.4 : 1 it had before
-- this line of work, in the safe direction.
--
-- The card names are German („Bedingungsbaum", „typisierte Tabelle", „Hinweis")
-- rather than the wire `type` values, matching the rest of this body: the skill
-- is German prose and calls a thing what a reader would call it. Backticks stay
-- for what the model literally writes — the `status` field, a tool name.
--
-- Guarded on the md5 of the body 0059 left behind, not on `created_by`: the
...
ON CONFLICT ("name") DO UPDATE
  SET "description" = EXCLUDED."description",
      "body" = EXCLUDED."body",
      "updated_at" = now()
  WHERE md5("platform_skills"."body") = '053d2692144151cd3d019b544079ad42';  -- pragma: allowlist secret
