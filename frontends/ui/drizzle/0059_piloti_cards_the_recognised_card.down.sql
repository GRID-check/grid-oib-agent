-- Revert the card judgement to the body 0058 left behind.
--
-- Guarded on the md5 of the body 0059 wrote, so a row edited through the
-- dashboard after 0059 ran is left exactly as it is — the same rule the forward
-- migration follows, for the same reason. `created_by = 'system'` is kept as a
-- second gate for consistency with 0055, 0057 and 0058: the md5 is what actually
...
-- names what the body teaches, and a description promising a section the body no
-- longer carries is worse than either half alone.

UPDATE "platform_skills"
   SET "description" = 'Vor dem Emittieren einer Karte laden: welche der allgemeinen Karten eine gewöhnliche Antwort wirklich besser macht und welche sie nur ein zweites Mal erzählt — Urteilskarte gegen ersten Satz, wann Kernaussagen verdient sind, ein Hinweis pro Antwort, brauchbare Anschlussfragen, Bedingungsbaum gegen Tabelle gegen Vergleich, der Rechenweg mit seinen Eingangswerten statt eines hingeschriebenen Ergebnisses, der Verfahrensablauf nur mit tragenden Stationen, und wie viele Karten zu viele sind. Gilt für jede fachliche Antwort, nicht nur für lange.',
...
Antwort; sie ist nie ein Stück davon, das woanders hingerutscht ist.',
 ...
 WHERE "name" = 'piloti-cards'
   ...
   AND md5("platform_skills"."body") = '053d2692144151cd3d019b544079ad42';  -- pragma: allowlist secret
