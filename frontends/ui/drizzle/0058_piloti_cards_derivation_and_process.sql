-- The card judgement learns the two shapes added with it.
--
-- `calculation` and `process_map` landed with a trigger line each in
-- `render_card_doctrine()`, which is all the tool description may carry: it is
-- paid on every turn whether or not a card is emitted, and a ceiling test fails
-- if craft drifts back into it. The paragraphs that say when each card actually
-- earns its place belong here, in the skill that is applied on every answering
-- turn anyway and that a platform owner can edit without a deploy.
--
-- What the two paragraphs are FOR, since neither is obvious from the shape:
--
-- `calculation` has no result field on the wire — the renderer computes from the
-- operands. The paragraph has to say so, because a model that does not know it
-- will look for somewhere to put the answer, and the whole honesty property of
-- the card is that there is nowhere. It also has to say that the factor in
-- „2 × Steigung + Auftritt" belongs to the Bestimmung rather than to a
-- measurement, and that a missing operand is left missing rather than estimated
-- to make the arithmetic close.
--
-- `process_map` fails in the opposite direction: it is trivially easy to emit a
-- row of station names, which is the numbered list it replaced with a frame
-- drawn around it and a click that opens onto nothing. The paragraph makes the
-- payload the point, and forbids guessing `current_step` — telling a reader they
-- hold a Baubewilligung they may not hold is the one error this card can make
-- that the prose beside it cannot correct.
--
-- Neither card was added to `grid-cards`. Their shapes cost +1,114 and +693
-- tokens on EVERY turn to save a `describe_card` round-trip on the few turns
-- they fire — the same trade that got `norm_chain` and `typed_table` cut, at
-- three times the price.
--
-- Guarded on the md5 of the body 0056 left behind, not on `created_by`: the
...
VALUES (
  'piloti-cards',
  'Vor dem Emittieren einer Karte laden: welche der allgemeinen Karten eine gewöhnliche Antwort wirklich besser macht und welche sie nur ein zweites Mal erzählt — Urteilskarte gegen ersten Satz, wann Kernaussagen verdient sind, ein Hinweis pro Antwort, brauchbare Anschlussfragen, Bedingungsbaum gegen Tabelle gegen Vergleich, der Rechenweg mit seinen Eingangswerten statt eines hingeschriebenen Ergebnisses, der Verfahrensablauf nur mit tragenden Stationen, und wie viele Karten zu viele sind. Gilt für jede fachliche Antwort, nicht nur für lange.',
...
## Verfahrensablauf: nur wenn die Stationen etwas tragen
...
ON CONFLICT ("name") DO UPDATE
  SET "description" = EXCLUDED."description",
      "body" = EXCLUDED."body",
      "updated_at" = now()
  WHERE md5("platform_skills"."body") = '2c76419e0dfdabb7650ad726a34294d0';  -- pragma: allowlist secret
