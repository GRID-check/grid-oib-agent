-- The two cards the doctrine redirects to become reachable without a round trip.
--
-- `grid-cards` does two things at once, and only the first is obvious. It states
-- the skill author's card preference, and — because `SkillRuntime` appends
-- `render_card_details()` for exactly those types when the body is delivered
-- (`skills/runtime.py::_preferred_cards_block`) — it also decides which SHAPES
-- are already in context when the model reaches the moment of emitting. A card
-- whose shape is inlined can be emitted from where the model is standing; every
-- other card costs a `describe_card` call first.
--
-- The set inlined since 0054 is verdict_header, condition_tree, key_takeaways,
-- callout, follow_ups. Neither of the two cards this body spends the most words
-- redirecting TO was in it:
--
--   * `typed_table` — the shared doctrine sends "rows that are all true at once"
--     here, and „Die Vorfrage zum Bedingungsbaum" in the body below works the GK
--     4 Feuerwiderstand case through to exactly this card. So the model was told
--     to prefer it while holding `condition_tree`'s full shape and none of this
--     one's. The observed answer took the condition_tree.
--   * `process_map` — the card the „wie läuft das ab" transcript was supposed to
--     produce, and the one whose miss opened this line of work at all.
--
-- WHAT IT COSTS, measured with tiktoken `cl100k_base` over
-- `render_card_details()` and not estimated from characters: the five inlined
-- shapes are 1,199 tokens. `typed_table` adds 265 and `process_map` adds 693, so
-- the block goes to 2,157 — plus 958 tokens on every turn that delivers this
-- body, which `delivery: standard` makes every research turn.
--
-- That is not free and it is not obviously right, so the trade is written down
-- rather than assumed. What it buys back: `process_map` is 693 tokens paid once
-- per turn against a `describe_card` round trip — a whole LLM call plus the same
-- 693 tokens of shape in its result — paid on every turn that emits one. The
-- round trip is only cheaper while the card keeps not being emitted, which is
-- the failure being fixed. `typed_table` at 265 is the cheap half and the one
-- with two doctrine rows and a body section pointing at it.
--
-- What would make it wrong: if the fleet's turns are dominated by answers that
-- emit neither card, this is 958 tokens of dead weight per turn. The counter is
-- that the same argument applied to the five already inlined and they were
-- inlined anyway; `follow_ups` in particular is described in the doctrine as
-- closing a subject-matter answer BY DEFAULT, so its shape is wanted on nearly
-- every turn. If the number is judged too large, the smaller move is to inline
-- `typed_table` alone (+265) and leave `process_map` to `describe_card`, whose
-- L1 line already carries an imperative. This migration takes both, and the
-- reversal is `0061_..._reachable_shapes.down.sql`.
--
-- METADATA ONLY. The body and the description are restated verbatim from 0060 so
-- a fresh install gets one complete row, but `DO UPDATE` touches `metadata`
-- alone: nothing in the German prose changes here.
--
-- Guarded on the md5 of the body 0060 left behind, not on `created_by` — the
-- dashboard patches `body` and never touches `created_by`, so a hand-edited row
-- still reads `system` and must not be overwritten. Guarding a metadata change on
-- the BODY's hash is deliberate: an owner who has rewritten the prose owns the
-- whole row, card preferences included, and a preference list pointing at shapes
-- their body no longer discusses is worse than leaving their row alone. The
-- converse is accepted rather than overlooked: an owner who edited only
-- `grid-cards` and left the prose alone IS overwritten here, because a row
-- whose body is still ours reads as ours and no column records who last
-- touched the metadata. The rollback does not have that problem — it gates on
-- the exact list this migration wrote, so a re-chosen list survives it.
--
-- The second WHERE clause (`metadata IS DISTINCT FROM EXCLUDED.metadata`) is what
-- makes a re-run a true no-op rather than an idempotent write: without it every
-- replay of the migration would bump `updated_at` on a row it did not change.
...
Antwort; sie ist nie ein Stück davon, das woanders hingerutscht ist.',
  -- The ONE line this migration changes. `typed_table` and `process_map` join
  -- the inlined set; the other five are 0054's, in 0054's order.
  '{"grid-agents": "shallow_researcher", "grid-hidden": "true", "grid-cards": "verdict_header,condition_tree,typed_table,key_takeaways,callout,process_map,follow_ups"}'::jsonb,
  true,
  'standard',
  'system',
  NULL
...
ON CONFLICT ("name") DO UPDATE
  ...
  WHERE md5("platform_skills"."body") = '9662a746c7e877a70e5c814b4cbdecca'  -- pragma: allowlist secret
    AND "platform_skills"."metadata" IS DISTINCT FROM EXCLUDED."metadata";
