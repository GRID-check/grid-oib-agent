-- Revert the card judgement to the body 0059 left behind.
--
-- Guarded on the md5 of the body 0060 wrote, so a row edited through the
-- dashboard after 0060 ran is left exactly as it is — the same rule the forward
-- migration follows, for the same reason. `created_by = 'system'` is kept as a
-- second gate for consistency with 0055, 0057, 0058 and 0059: the md5 is what
-- actually protects an edited body, since the dashboard patches `body` and never
-- touches `created_by`, but a row somebody re-created by hand should not be
-- reverted either.
--
-- The description reverts with the body. They are one instruction: the L1 line
-- names what the body teaches, and a description promising three card sections
-- the body no longer carries is worse than either half alone.
...
Antwort; sie ist nie ein Stück davon, das woanders hingerutscht ist.',
 ...
 WHERE "name" = 'piloti-cards'
   ...
   AND md5("platform_skills"."body") = '9662a746c7e877a70e5c814b4cbdecca';  -- pragma: allowlist secret
