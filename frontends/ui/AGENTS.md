...
| Add a card type | Classify it in `CARD_INTERACTIVITY` (`features/grid-cards/card-decision.ts`). An interactive, system (`SYSTEM_CARD_TYPES`) or envelope type is also added to `SURFACE_EXCLUDED_LEAVES` in `cards/models.py` **and** `features/a2ui/catalog.tsx`: a leaf inside a surface has no message position for its decision to be keyed by | `task fe:types`; `tests/aiq_agent/cards/test_surface_excluded_parity.py` |
| Touch `features/a2ui` | Keep the catalog the card types (ADR-0065). It is built from the generated `gridCardSchema`, so a card type is added in `src/aiq_agent/cards/models.py` and reaches the catalog through `npm run generate:cards`; only `SURFACE_EXCLUDED_LEAVES` is added by hand, in both files together | `card-schemas` pre-commit hook; a type missing from the catalog is refused by `preflight` and drawn without A2UI |
