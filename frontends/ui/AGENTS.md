...
| Ship a user-visible surface | A `/dev/<name>` preview route, and a capture of it attached to the PR (`agent-browser` + `before-and-after`). Commit no PNGs | CI's **Require a capture for user-visible changes**. [`docs/ux/visual-screenshots.md`](../../docs/ux/visual-screenshots.md) |
