...
- **Optional WorkOS AuthKit authentication** (disabled by default)
...
│   ├── auth/           # WorkOS AuthKit compatibility adapter (useAuth, token helpers, types)
...
| Variable | Default | Description |
|----------|---------|-------------|
| `REQUIRE_AUTH` | `false` | Set to `true` to require WorkOS AuthKit login |
| `WORKOS_CLIENT_ID` | - | WorkOS client ID (required if auth enabled) |
| `WORKOS_API_KEY` | - | WorkOS API key (required if auth enabled) |
| `WORKOS_REDIRECT_URI` | - | AuthKit callback URL, e.g. `http://localhost:3000/api/auth/callback` (required if auth enabled) |
| `WORKOS_COOKIE_PASSWORD` | - | 32-byte random string used to encrypt the AuthKit session cookie (required if auth enabled) |
...
Set these variables to enable WorkOS AuthKit:
...
See `.env.example` for the full list of available frontend variables.
...
Authentication is **disabled by default**. When disabled, the UI runs as a "Default User" and no login is required. To require login, enable WorkOS AuthKit by setting `REQUIRE_AUTH=true` and configuring the WorkOS variables in `.env.local`.

### Architecture

```
src/adapters/auth/
├── use-auth.ts      # Provider-agnostic useAuth() hook wrapping AuthKit
├── types.ts         # Client auth types
├── token.ts         # JWT exp helper for token rotation
└── index.ts         # Re-exports

src/lib/auth/
├── session.ts       # Server GridSession helper wrapping withAuth()/refreshSession()
├── require-auth.ts  # requireAuthorizedSession() redirects to onboarding
└── types.ts         # GridSession / AuthorizedSession types

src/lib/authz/
└── projects.ts      # project_members lookup + admin override
```

- `src/app/providers.tsx` wraps the app with `AuthKitProvider`.
- `src/proxy.ts` uses `authkitMiddleware` to keep the encrypted session cookie and access token current.
- API routes use `withAuth()` and forward the raw WorkOS access token to the Python agent.
- `useAuth()` returns `{ isAuthenticated, isLoading, user, accessToken, idToken, organizationId, role, permissions, signIn, signOut, getAccessToken }`. `idToken` is mapped to the WorkOS access token for backward compatibility.

### Enabling WorkOS AuthKit

Set these environment variables in `frontends/ui/.env.local`:
...
Set `REQUIRE_AUTH=false` (or leave it unset). The app falls back to a "Default User" identity and no sign-in flow is triggered.
...
When auth is disabled, `useAuth()` returns `isAuthenticated: true` with a default user.
