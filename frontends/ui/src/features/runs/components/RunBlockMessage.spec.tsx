...
vi.mock('./RunBlock', () => ({
  RunBlock: ({ ledger, title, live }: { ledger: { status: string }; title?: string | null; live?: boolean }) => (
    <div data-testid="run-block" data-status={ledger.status} data-live={live ? 'true' : undefined}>
      {title}
    </div>
  ),
}))
...
describe('RunBlockMessage', () => {
  it('renders the block from the hook’s ledger with the message’s title', () => {
    vi.mocked(useRunLedger).mockReturnValue({ ledger: ledger('laeuft'), live: true })
  ...
  it('renders the report beneath the block once the run is finished', () => {
    vi.mocked(useRunLedger).mockReturnValue({ ledger: ledger('fertig'), live: false })
  ...
  it('renders the report of an interrupted run too — the block says one was written', () => {
    vi.mocked(useRunLedger).mockReturnValue({ ledger: ledger('unterbrochen'), live: false })
