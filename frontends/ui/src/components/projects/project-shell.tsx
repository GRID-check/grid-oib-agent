        ...
        <aside className="hidden border-r border-base bg-surface-raised-30 p-4 lg:block">
          <Flex direction="col" gap="5" className="sticky top-20">
            ...
            <nav className="flex flex-col gap-2" aria-label="Project navigation">
              {projectNavItems.map(({ label, href, icon: Icon }) => (
                <Link
                  key={label}
                  href={`/projects/${projectId}${href}`}
                  className="group flex items-center gap-3 rounded-2xl border border-transparent px-3 py-3 text-sm font-semibold text-primary transition hover:border-base hover:bg-surface-base"
                >
                  <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-surface-sunken text-brand transition group-hover:bg-surface-raised-50">
