 ...
 *
 * The caller must have platform access (the internal route declares
 * `crossTenant`), because both the aggregate and the inbox writes span tenants.
 */
export async function runStorageAlertSweep(): Promise<StorageAlertSweepResult> {
  ...
  for (const [organizationId, usage] of usageByOrg) {
    try {
      outcomes.push(await evaluateStorageAlert(organizationId, usage.bytes, { thresholdPercent }))
    } catch (error) {
