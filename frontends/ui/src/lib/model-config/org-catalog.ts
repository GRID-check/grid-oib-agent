...
import { fetchModelCatalog, type OpenRouterModel } from './openrouter'
 ...
 */
export async function getCatalogForOrg(organizationId: string): Promise<OrgModelCatalog> {
  const credential = await resolveActiveCredentialForBackend(organizationId)

  if (!credential) {
    return { models: await fetchModelCatalog(), source: 'openrouter', provider: null, validation: 'full' }
  }
  if (credential.provider === 'openrouter') {
    // Same public catalog, full metadata — traffic just bills to the org key.
    return { models: await fetchModelCatalog(), source: 'byok', provider: 'openrouter', validation: 'full' }
  }
