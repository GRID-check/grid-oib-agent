...
describe('transitionDocumentVersion — effects', () => {
  ...
  it('dispatches nothing on publish — the ingest slot is a no-op in this slice', async () => {
    const published = version({ state: 'published' })
    vi.mocked(findDocumentVersion).mockResolvedValue(version({ state: 'approved' }))
    ...
    // Slice 4 is what turns `ingestPublished` on, and only together with the
    // labelled-citation work. Until then a published version has no chunks.
