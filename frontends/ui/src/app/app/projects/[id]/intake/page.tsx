...
export default async function IntakePage({ params }: IntakePageProps): Promise<JSX.Element> {
  ...
  // fact/goal/unknown, drop only the invalid entries) rather than discarded whole:
  // discarding it silently would let the next Save overwrite a still-good brief with
  // only the freshly-typed answers. When anything was dropped the wizard shows an
  // honest banner instead of blanking.
  const { profile: initialProfile, dropped } = salvageProjectProfile(project.profile)
  ...
  // Banner variant: 'full' when nothing salvageable remained (create mode, but the
  // stored brief WILL be replaced on save), 'partial' when some parts were dropped.
  const salvageNotice: 'partial' | 'full' | null =
    dropped.length === 0 ? null : initialProfile === null ? 'full' : 'partial'
